...
<div class="party-page">
  <header class="hero section-card">
    ...
    <div class="origin-policy">
      ...
      <a href={`/${data.tenantSlug}/app/functions/f07`}
        ><span>Client / customer</span><b>F07 Sales & Commercial</b></a
      >
      <a href={`/${data.tenantSlug}/app/functions/f09`}
        ><span>Supplier / subcontractor</span><b>F09 Procurement & Suppliers</b></a
      >
      <a href={`/${data.tenantSlug}/app/functions/f15`}
        ><span>Employee / worker</span><b>F15 People & Workforce</b></a
      >
      <a href={`/${data.tenantSlug}/app/functions/f19`}
        ><span>Legal / regulator role</span><b>F19 Legal & Secretariat</b></a
      >
    </div>
  </header>
  ...
  <form class="filters section-card" method="GET">
    <label class="search"
      >Search<input
        ...
        placeholder="Name, registration, origin or identifier"
      /></label
    >
    <label
      >Party type<select name="type"
        ><option value="">All types</option><option
          value="PERSON"
          selected={data.filters.type === 'PERSON'}>Person</option
    ...
    <label
      >Status<select name="status"
        ><option value="">All statuses</option><option
          value="ACTIVE"
          selected={data.filters.status === 'ACTIVE'}>Active</option
    ...
    <section class="register section-card">
      <div class="panel-heading">
        <div>
          ...
          <h2>{data.parties.length} matching identities</h2>
        </div>
      </div>
        ...
        {#each data.parties as party}
          <a class:active={data.selected?.id === party.id} href={partyHref(party.id)}>
            <span class:person={party.partyType === 'PERSON'} class="type-icon"
              >{party.partyType === 'PERSON' ? 'P' : 'O'}</span
            >
            <span class="party-copy"
              ><strong>{party.displayName}</strong><small
                >{party.partyType === 'PERSON'
                  ? 'Person'
                  : party.isLegalEntity
                    ? 'Legal entity'
                    : 'Organisation'} · {party.status}</small
              ></span
            >
      ...
      <div class="creation-rule">
        ...
        <p>
          Start client, supplier and employee onboarding in the owning business function. NuBlox
          resolves or creates the canonical Party behind that workflow.
        </p>
      </div>
    </section>
    ...
    <main class="inspector">
      {#if data.selected}
        <section class="identity-card section-card">
          <div class="identity-heading">
            ...
            <div class="identity-actions">
              <div class="identity-badges">
                <span>{data.selected.status}</span><span>v{data.selected.version}</span
              ...
              <TaskContextButton
                ...
                workspaceFunctionId={data.selected.originFunctionId?.startsWith('F')
                  ? data.selected.originFunctionId
                  : null}
              />
            </div>
          </div>
          ...
          <section class="provenance">
            <div>
              <span class="eyebrow">Origination & stewardship</span>
              <h3>
                {functionNames[data.selected.originFunctionId ?? 'PLATFORM'] ??
                  data.selected.originFunctionId ??
                  'Legacy / unclassified'}
              </h3>
            </div>
            <div class="provenance-grid">
              <span
                ><small>Origin function</small><strong
                  >{data.selected.originFunctionId ?? 'Legacy'}</strong
                ></span
              >
              <span
                ><small>Origin object</small><strong>{data.selected.originObjectType ?? '—'}</strong
                ></span
              >
              <span
                ><small>Steward</small><strong
                  >{functionNames[data.selected.stewardFunctionId ?? 'PLATFORM'] ??
                    data.selected.stewardFunctionId ??
                    '—'}</strong
                ></span
              >
              <span
                ><small>Created</small><strong
                  >{new Date(data.selected.createdAt).toLocaleDateString('en-GB')}</strong
                ></span
              >
            </div>
          ...
          {#if data.selected.partyType === 'ORGANISATION' && data.authority.canSteward}
            <a
              class="editor-link"
              href={`/${data.tenantSlug}/app/admin/master-data/organisations?organisation=${data.selected.id}`}
            >
        ...
        <section class="relationships section-card">
          ...
          <p class="section-note">
            Relationships are displayed here for identity resolution. Their business lifecycle
            belongs to the owning function.
          </p>
            ...
            {#each data.relationships as relationship}
              <article>
                ...
                <div class="relationship-copy">
                  <strong>{relationshipParty(relationship)}</strong><small
                    >{relationship.relationshipType} · {relationship.contextType}</small
                  >
                </div>
                ...
                {#if relationship.homeHref}
                  <a class="home-link" href={relationship.homeHref}
                    >Open {relationship.homeFunctionName} →</a
                  >
                {:else}
      ...
      {:else}
        <section class="section-card empty-state">
          ...
          <p>
            New identities will enter the directory through their owning client, supplier, employee
            or legal workflow—not from this administration page.
          </p>
        </section>
      {/if}
    </main>
