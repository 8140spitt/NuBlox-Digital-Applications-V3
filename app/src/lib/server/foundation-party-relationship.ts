...
async function evidence(context: CommandContext, row: PartyRelationship, action: string, executor: DbExecutor, fromState?: string) {
...
export async function listPartyRelationships(context: CommandContext) {
  ...
  return queryRows<RowDataPacket & PartyRelationship>(
    selectRelationship + ' WHERE r.tenant_id = ? ORDER BY fp.display_name, tp.display_name, r.relationship_type',
    [context.tenantId]
  );
}

export async function createPartyRelationship(context: CommandContext, input: PartyRelationshipInput) {
  assertPermission(context, 'party.relationship.manage');
  if (input.fromPartyId === input.toPartyId) throw new Error('A Party Relationship requires two different Parties.');
  ...
  return dbTransaction(async (connection) => {
    ...
    const duplicate = await queryOne<RowDataPacket & { id: string }>(
      ...
      [context.tenantId, input.fromPartyId, input.toPartyId, relationshipType, contextType, contextId],
      connection
    );
    if (duplicate) throw new Error('An open Party Relationship of this type already exists for this context.');
    ...
    await executeMutation(
      ...
      [id, context.tenantId, input.fromPartyId, input.toPartyId, relationshipType, contextType, contextId, validFrom, validTo, timestamp, timestamp],
      connection
    );
    await evidence(context, await getRow(context, id, connection), 'PARTY_RELATIONSHIP_CREATED', connection);
    return id;
  });
}
...
async function transition(context: CommandContext, id: string, expectedVersion: number, allowedFrom: string[], toState: string, action: string) {
  ...
  return dbTransaction(async (connection) => {
    ...
    if (current.version !== expectedVersion) throw new Error('This Party Relationship changed after you opened it.');
    if (!allowedFrom.includes(current.status)) throw new Error(`Party Relationship cannot move from ${current.status} to ${toState}.`);
    const timestamp = now();
    ...
    if (result.affectedRows !== 1) throw new Error('Concurrent Party Relationship change detected.');
...
export const activatePartyRelationship = (context: CommandContext, id: string, version: number) =>
  transition(context, id, version, ['PROPOSED','SUSPENDED'], 'ACTIVE', 'PARTY_RELATIONSHIP_ACTIVATED');
...
export const endPartyRelationship = (context: CommandContext, id: string, version: number) =>
  transition(context, id, version, ['PROPOSED','ACTIVE','SUSPENDED'], 'ENDED', 'PARTY_RELATIONSHIP_ENDED');
