...
export async function listGovernanceMeetings(
  context: CommandContext,
  meetingType = 'STRATEGIC_REVIEW'
) {
  assertPermission(context, 'strategy.review.read');
  return queryRows<RowDataPacket & GovernanceMeeting>(
    'SELECT id,meeting_ref AS meetingRef,meeting_type AS meetingType,governance_context_type AS governanceContextType,governance_context_id AS governanceContextId,scheduled_at AS scheduledAt,actual_started_at AS actualStartedAt,completed_at AS completedAt,location_channel AS locationChannel,quorum_required AS quorumRequired,status,aggregate_version AS aggregateVersion,minutes_summary AS minutesSummary,next_review_at AS nextReviewAt,updated_at AS updatedAt FROM governance_meetings WHERE tenant_id=? AND meeting_type=? ORDER BY scheduled_at DESC',
    [context.tenantId, code(meetingType, 'Meeting type', 64)]
  );
}
export async function listMeetingAttendees(context: CommandContext, meetingId: string) {
  assertPermission(context, 'strategy.review.read');
  await getMeeting(context, meetingId);
  return queryRows<RowDataPacket & GovernanceMeetingAttendee>(
...
export async function listStrategicReviewFindings(context: CommandContext, meetingId: string) {
  assertPermission(context, 'strategy.review.read');
  await getMeeting(context, meetingId);
  return queryRows<RowDataPacket & StrategicReviewFinding>(
    'SELECT id,finding_type AS findingType,subject_type AS subjectType,subject_id AS subjectId,subject_version AS subjectVersion,finding,recommendation,created_at AS createdAt FROM strategic_review_findings WHERE tenant_id=? AND meeting_id=? ORDER BY created_at,id',
    [context.tenantId, meetingId]
  );
}
export async function listMeetingDecisionIds(context: CommandContext, meetingId: string) {
  ...
  return queryRows<RowDataPacket & { decisionId: string }>(
    'SELECT d.decision_id AS decisionId FROM governance_meeting_decisions d JOIN governance_meetings m ON m.id=d.meeting_id WHERE d.meeting_id=? AND m.tenant_id=? ORDER BY d.decision_id',
...
export async function listMeetingActionIds(context: CommandContext, meetingId: string) {
  ...
  return queryRows<RowDataPacket & { workItemId: string }>(
    'SELECT a.work_item_id AS workItemId FROM governance_meeting_actions a JOIN governance_meetings m ON m.id=a.meeting_id WHERE a.meeting_id=? AND m.tenant_id=? ORDER BY a.work_item_id',
    [meetingId, context.tenantId]
  );
}
...
export async function createStrategicReview(
  ...
  if (!Number.isInteger(input.quorumRequired) || input.quorumRequired < 1)
    throw new Error('Strategic Review quorum must be a positive whole number.');
...
export async function recordMeetingAttendance(
  ...
  return dbTransaction(async (connection) => {
    ...
    if (!['SCHEDULED', 'CONVENED'].includes(meeting.status))
      throw new Error('Attendance can only be recorded before or during a Governance Meeting.');
    const result = await executeMutation(
      'UPDATE governance_meeting_attendees SET attendance_status=? WHERE meeting_id=? AND party_id=?',
      [status, meeting.id, partyId],
      connection
    );
    if (result.affectedRows !== 1) throw new Error('Meeting attendee not found.');
  });
}

export async function conveneStrategicReview(
  context: CommandContext,
  meetingId: string,
  ...
  return dbTransaction(async (connection) => {
    ...
    if (meeting.aggregateVersion !== expectedAggregateVersion)
      throw new Error('This Governance Meeting changed after you opened it.');
    if (meeting.status !== 'SCHEDULED')
      throw new Error('Only a scheduled Strategic Review can be convened.');
    ...
    if (Number(present?.count ?? 0) < meeting.quorumRequired)
      throw new Error('Strategic Review quorum has not been met.');
    const started = now();
    const result = await executeMutation(
    ...
    if (result.affectedRows !== 1)
      throw new Error('Concurrent Governance Meeting change detected.');
    const updated = await getMeeting(context, meeting.id, connection);
    await evidence(
      context,
      updated,
      'STRATEGIC_REVIEW_CONVENED',
      'SCHEDULED',
      'CONVENED',
...
export async function addStrategicReviewFinding(
  context: CommandContext,
  meetingId: string,
  input: {
    findingType: string;
  ...
  return dbTransaction(async (connection) => {
    ...
    if (meeting.status !== 'CONVENED')
      throw new Error('Findings can only be recorded during a convened Strategic Review.');
    const id = randomUUID();
    await executeMutation(
      'INSERT INTO strategic_review_findings (id,tenant_id,meeting_id,finding_type,subject_type,subject_id,subject_version,finding,recommendation,created_by_party_id,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)',
      [
        id,
        context.tenantId,
        meeting.id,
        code(input.findingType, 'Finding type', 64),
        code(input.subjectType, 'Finding subject type', 64),
        required(input.subjectId, 'Finding subject ID'),
        input.subjectVersion?.trim() || null,
        required(input.finding, 'Review finding'),
        required(input.recommendation, 'Review recommendation'),
        context.actorPartyId,
        now()
      ],
      connection
...
export async function recordStrategicReviewDecision(
  context: CommandContext,
  meetingId: string,
  input: {
    decisionType: string;
  ...
  if (meeting.status !== 'CONVENED')
    throw new Error('Decisions can only be linked while the Strategic Review is convened.');
  ...
  await dbTransaction(async (connection) => {
    ...
    if (locked.status !== 'CONVENED') throw new Error('Strategic Review is no longer convened.');
    await executeMutation(
...
export async function createStrategicReviewAction(
  ...
  const meeting = await getMeeting(context, meetingId);
  if (meeting.status !== 'CONVENED')
    throw new Error('Actions can only be created while the Strategic Review is convened.');
  const workflowId = await createWorkflowInstance(context, {
    definitionKey: 'STRATEGIC_REVIEW_ACTION',
    definitionVersion: '1',
  ...
  const workItemId = await createWorkItem(context, workflowId, {
    workType: 'STRATEGIC_REVIEW_ACTION',
    title: input.title,
...
export async function completeStrategicReview(
  context: CommandContext,
  meetingId: string,
  expectedAggregateVersion: number,
  ...
  return dbTransaction(async (connection) => {
    ...
    if (meeting.aggregateVersion !== expectedAggregateVersion)
      throw new Error('This Governance Meeting changed after you opened it.');
    if (meeting.status !== 'CONVENED')
      throw new Error('Only a convened Strategic Review can be completed.');
    const openAgenda = await queryOne<RowDataPacket & { count: number }>(
      "SELECT COUNT(*) AS count FROM governance_meeting_agenda_items WHERE meeting_id=? AND status NOT IN ('CLOSED','DEFERRED','WITHDRAWN')",
      [meeting.id],
      connection
    );
    ...
    const result = await executeMutation(
      ...
      [
        completed,
        required(input.minutesSummary, 'Strategic Review minutes summary'),
        nextReviewAt,
        completed,
        meeting.id,
        context.tenantId,
    ...
    if (result.affectedRows !== 1)
      throw new Error('Concurrent Governance Meeting completion detected.');
    const updated = await getMeeting(context, meeting.id, connection);
    await evidence(
      context,
      updated,
