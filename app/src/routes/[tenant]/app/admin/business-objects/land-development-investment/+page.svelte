<script lang="ts">
  import {
    ...
    landDevelopmentInvestmentRelationships,
    landDevelopmentInvestmentRules
  } from '$lib/data/land-development-investment-model';

  let { data } = $props();
  const names = new Map(landDevelopmentInvestmentModel.map((x) => [x.modelId, x.canonicalName]));
  const external = new Map([
    ['BE-PROPERTY','Property'],['CBO-CONTRACT','Contract'],['CBO-PARTY','Party'],['CBO-SITE','Site'],
    ['CBO-INFORMATION-CONTAINER','Information Container'],['LEGAL-OBLIGATION','Legal Obligation'],
    ['WORK-DECISION','Decision'],['CBO-PROJECT','Project']
  ]);
  const nameFor=(id:string)=>names.get(id)??external.get(id)??id;
</script>

<svelte:head><title>Land & Development Semantics · NuBlox</title></svelte:head>

<div class="page">
  <header class="hero card">
    <div>
      <span class="eyebrow">Development and investment model</span>
      <h1>Land, Development, Investment & Property Acquisition</h1>
      <p>Opportunities, business cases, land/property interests, appraisal, planning and funding evidence around canonical land, property, Party, Contract and Project identities.</p>
    </div>
    <div class="actions">
      ...
      <strong>{landDevelopmentInvestmentModel.length} governed constructs</strong>
      <span>{landDevelopmentInvestmentRelationships.length} relationships</span>
    </div>
  </header>

  <section class="card chain">
    <div><span>Opportunity</span><strong>Development Opportunity → Business Case</strong></div>
    <div><span>Control</span><strong>Option / Property Interest / Parcel</strong></div>
    <div><span>Appraisal</span><strong>Site / Development / Valuation</strong></div>
    <div><span>Planning</span><strong>Application → Consent → Conditions</strong></div>
    <div><span>Decision</span><strong>Funding Evidence → Decision → Project</strong></div>
  </section>

  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Semantic contracts</span><h2>Governed development constructs</h2></div><p>Development-specific context surrounds shared enterprise/spatial identities instead of duplicating them.</p></div>
    <div class="grid">
      {#each landDevelopmentInvestmentModel as item,i}
        <details open={i<4}>
  ...
  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Relationship graph</span><h2>Land-to-decision provenance</h2></div></div>
    <div class="table"><table><thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Governance</th></tr></thead><tbody>
      {#each landDevelopmentInvestmentRelationships as r}<tr><td><code>{r.id}</code></td><td>{nameFor(r.from)}</td><td>{r.predicate}</td><td>{nameFor(r.to)}</td><td>{r.governance}</td></tr>{/each}
    </tbody></table></div>
  </section>

  <section class="card content rules">
    <span class="eyebrow">Non-negotiable rules</span><h2>What NuBlox must preserve</h2>
    <ol>{#each landDevelopmentInvestmentRules as x}<li>{x}</li>{/each}</ol>
  </section>
</div>
