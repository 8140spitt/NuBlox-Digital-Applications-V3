<script lang="ts">
  ...
  const external = new Map([
    ['CBO-SITE','Site'],['BE-ZONE','Zone'],['DEL-STAGE-ASSIGNMENT','Delivery Stage Assignment'],
    ['DEL-WORK-PACKAGE','Work Package'],['DEL-PROGRESS-RECORD','Progress Record'],
    ['SEC-PHYSICAL-ACCESS-EVENT','Physical Access Event'],['HCM-TIME-ENTRY','Time Entry'],
    ['CBO-ASSET','Asset'],['INV-MOVEMENT','Inventory Movement'],['LOG-DELIVERY','Delivery'],
    ['CBO-INFORMATION-CONTAINER','Information Container'],['INFO-DESIGN-REVIEW','Design Review'],
    ['QHSE-PERMIT','Permit to Work'],['QHSE-ISOLATION','Isolation'],
    ['EVID-EVIDENCE-ITEM','Evidence Item'],['WORK-FOLLOW-UP-ACTION','Decision Action'],
...
<div class="page">
  <header class="hero card">
    <div>
      ...
      <p>Work areas, mobilisation, daily records, labour/plant/material evidence, temporary works, permits, instructions, constraints, completion and handover readiness around shared project, site and assurance truth.</p>
    </div>
    <div class="actions">
  ...
  <section class="card chain">
    ...
    <div><span>Control</span><strong>Temporary Works / Permit / Isolation / Constraint</strong></div>
    <div><span>Complete</span><strong>Completion → Handover Readiness</strong></div>
  </section>

  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Semantic contracts</span><h2>Governed field-execution constructs</h2></div><p>Field operations capture execution truth and evidence while reusing shared enterprise identities.</p></div>
    <div class="grid">
  ...
  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Relationship graph</span><h2>Field execution provenance</h2></div></div>
    <div class="table"><table><thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Governance</th></tr></thead><tbody>
      {#each siteFieldOperationsRelationships as r}<tr><td><code>{r.id}</code></td><td>{nameFor(r.from)}</td><td>{r.predicate}</td><td>{nameFor(r.to)}</td><td>{r.governance}</td></tr>{/each}
    </tbody></table></div>
  </section>

  <section class="card content rules">
    <span class="eyebrow">Non-negotiable rules</span><h2>What NuBlox must preserve</h2>
    <ol>{#each siteFieldOperationsRules as x}<li>{x}</li>{/each}</ol>
  </section>
</div>
