...
Wave 0 product-UI activation is active. Public `/register` now performs progressive, metadata-driven Tenant onboarding. Registration captures a governed Tenant Business Profile (industry classification, size tier, employee/legal-entity scale, country/language and operating model), resolves versioned configuration templates, retains exact provisioning evidence, activates mapped Industry Solutions, and then creates the identity/organisation spine atomically. The initial CBE mapping resolves `NUBLOX_INDUSTRY:CBE` to the Construction & Built Environment Industry Solution while preserving all 29 Core Business Functions. The Tenant configuration provenance is visible at `/{tenantSlug}/app/tenant-configuration`. Public `/register` now performs atomic Tenant onboarding: Tenant → TENANT Party → Organisation/root Unit plus registrant EMPLOYEE Party → Person → application identity/membership → Tenant administrator access, without fabricating a Position. Tenant-first routing provides two explicit physical Tenant route roots: private `/{tenantSlug}/app` and public `/{tenantSlug}/public`. Careers/candidate surfaces sit beneath the public root; employee session cookies are scoped to the app root; and protected requests require URL TenantId/session TenantId agreement. The authentication boundary now includes verified-email registration, expiring one-time verification/password-reset challenges, retryable security-message delivery, password-reset session revocation, persistent hashed anti-abuse throttling, tenant-scoped TOTP MFA/recovery codes, active-session administration, enforceable Tenant authentication policy and recent-MFA step-up for sensitive security administration. The tenant shell, secure application session boundary, HCM-resolved My Function navigation, live My Work, Organisation/Person/Position administration, governed access request/review/administration, HCM Position/Function-assignment/reporting administration, competence administration, a permission-gated Lifecycle/Decision/Evidence read workspace, and a governed Information/Revision/Representation workspace are implemented. Information release is bound to the exact canonical object/revision and requires an effective Authority-backed Decision. Managed content storage, control mutation workflows and Deliverable/Issue/Acceptance are still required before the shared platform is end-to-end user-operable. Change/Configuration/Baseline now has a governed tenant workspace with lifecycle-aware commands from Change assessment through implementation/verification/closure, plus Configuration Item, Baseline and Effectivity control; Change approval and Baseline establishment require effective Authority-backed Decisions. The completed PTC evidence pass has since exposed additional shared-control gaps accepted by ADR-0006, so the relevant shared engines are now correctly treated as partial until P1.8 is implemented.
...
- Tenant;
- Party;
- canonical Party Types: `TENANT`, `EMPLOYEE`, `CLIENT`, `VENDOR_SUPPLIER`;
- authoritative Tenant Party binding;
- Person;
- Organisation;
- Organisation Unit;
- Job Profile;
...
- public Tenant registration with verified-email gate;
- expiring verification and password-recovery challenges;
- identity-message outbox and retryable dispatcher;
...
- Extension Definition / Package Version / Compatibility / Reconciliation — **USER-EXECUTABLE FOUNDATION IMPLEMENTED**: governed extension identity, immutable checksum-backed package versions, typed package components, explicit platform-version compatibility assessments, component-level upgrade reconciliation with BLOCKED/RESOLVED derivation, migration 0036, separate extension permissions, audit/outbox, MySQL proof and `/app/extensions` workspace; signed provenance/dependency resolution continue as hardening and environment promotion remains WTR-021;
- Configuration Environment / Baseline / Change Set / Promotion Run / Conflict — **USER-EXECUTABLE FOUNDATION IMPLEMENTED**: explicit development/integration/test/preproduction/production environment identities, draft-to-frozen checksum-backed baselines, exact administrative change items, frozen change-set checksum with Decision-bound approval, mapping/rollback hashes, stale-target and in-flight drift protection, one-active-target promotion guard, per-item execution results, Decision-backed conflict disposition, successful before/after target baseline proof, migration 0037, separate configuration-promotion permissions, audit/outbox, MySQL journey proof and `/app/configuration-promotion` workspace; automated target-specific apply/rollback adapters continue as hardening;
- Retention Policy / Rule / Hold / Disposition Schedule / Run / Item Result / Archive / Restore / Destruction Evidence — **USER-EXECUTABLE FOUNDATION IMPLEMENTED**: versioned draft→frozen→Decision-approved active policies with deterministic checksums and atomic prior-version supersession; hold authority with explicit archive/destruction blocking and Decision-backed release; deterministic selection evidence and one-active-run-per-rule control; hold-aware disposition results; archive provenance; Decision-backed restore; Decision-backed destruction with separate metadata/content outcomes and approval required after the latest released blocking Hold; migration 0038; separate records-retention permissions; audit/outbox; MySQL journey proof and `/app/records-retention` workspace; automated candidate evaluation/scheduling workers and physical storage adapters continue as hardening.
