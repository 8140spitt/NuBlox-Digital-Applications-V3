...
function required(v:string,l:string){const c=v.trim();if(!c)throw new Error(l+' is required.');return c;}
function code(v:string,l:string,max=191){const c=required(v,l).toUpperCase();if(c.length>max||!/^[A-Z0-9][A-Z0-9._:-]*$/.test(c))throw new Error(l+' contains unsupported characters.');return c;}
function optionalCode(v:string|undefined,l:string,max=191){return v?.trim()?code(v,l,max):null;}
function subject(type?:string,id?:string){const t=type?.trim()?code(type,'Information subject type',64):null;const i=id?.trim()||null;if(Boolean(t)!==Boolean(i))throw new Error('Information subject type and subject ID must be supplied together.');return{subjectType:t,subjectId:i};}
function hash(algorithm:string,value:string){const a=code(algorithm,'Hash algorithm',32),h=required(value,'Content hash').toLowerCase();if(a==='SHA256'&&!/^[a-f0-9]{64}$/.test(h))throw new Error('SHA256 content hash must contain 64 hexadecimal characters.');if(a==='SHA512'&&!/^[a-f0-9]{128}$/.test(h))throw new Error('SHA512 content hash must contain 128 hexadecimal characters.');return{algorithm:a,value:h};}
async function activeParty(c:CommandContext,id:string,e:DbExecutor){const r=await queryOne<RowDataPacket&{id:string}>("SELECT id FROM parties WHERE id = ? AND tenant_id = ? AND status = 'ACTIVE'",[id,c.tenantId],e);if(!r)throw new Error('Active Party not found.');}
async function getContainer(c:CommandContext,id:string,e?:DbExecutor,lock=false){const r=await queryOne<RowDataPacket&InformationContainer>(containerSelect+' WHERE c.id = ? AND c.tenant_id = ?'+(lock?' FOR UPDATE':''),[id,c.tenantId],e);if(!r)throw new Error('Information Container not found.');return r;}
async function getRevision(c:CommandContext,containerId:string,revisionNo:number,e?:DbExecutor,lock=false){const r=await queryOne<RowDataPacket&InformationRevision>(revisionSelect+' WHERE tenant_id = ? AND container_id = ? AND revision_no = ?'+(lock?' FOR UPDATE':''),[c.tenantId,containerId,revisionNo],e);if(!r)throw new Error('Information Revision not found.');return r;}
async function evidence(c:CommandContext,x:InformationContainer,type:string,from:string|null,to:string,p:Record<string,unknown>,e:DbExecutor){await recordPlatformAudit(c,{aggregateId:'AGG-07-INFORMATION',objectType:'information_container',objectId:x.id,action:type,fromState:from??undefined,toState:to},e);await emitBusinessEvent(c,{aggregateId:'AGG-07-INFORMATION',aggregateType:'InformationContainer',aggregateObjectId:x.id,aggregateVersion:x.aggregateVersion,eventType:type,topic:'nublox.information.container',payload:p},e);}

export async function listInformationContainers(c:CommandContext,input:{containerType?:string;subjectType?:string;subjectId?:string;status?:string}={}){assertPermission(c,'information.container.read');const clauses=['c.tenant_id = ?'];const params:unknown[]=[c.tenantId];if(input.containerType){clauses.push('c.container_type = ?');params.push(code(input.containerType,'Container type',64));}if(input.subjectType){clauses.push('c.subject_type = ?');params.push(code(input.subjectType,'Subject type',64));}if(input.subjectId){clauses.push('c.subject_id = ?');params.push(input.subjectId);}if(input.status){clauses.push('c.status = ?');params.push(code(input.status,'Container status',32));}return queryRows<RowDataPacket&InformationContainer>(containerSelect+' WHERE '+clauses.join(' AND ')+' ORDER BY c.updated_at DESC, c.container_ref',params);}
...
export async function addInformationRepresentation(c:CommandContext,containerId:string,expected:number,input:{representationType:string;contentReference:string;contentMediaType?:string;sourceFilename?:string;hashAlgorithm:string;contentHash:string}){assertPermission(c,'information.container.manage');const h=hash(input.hashAlgorithm,input.contentHash);return dbTransaction(async e=>{const current=await getContainer(c,containerId,e,true);if(current.aggregateVersion!==expected)throw new Error('This Information Container changed after you opened it.');const rev=await getRevision(c,current.id,current.currentRevisionNo,e,true);if(rev.lifecycleStatus!=='WORKING')throw new Error('Representations cannot be changed after revision submission.');await executeMutation('INSERT INTO information_representations (id, tenant_id, container_id, revision_id, representation_type, content_reference, content_media_type, source_filename, hash_algorithm, content_hash, created_by_party_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',[randomUUID(),c.tenantId,current.id,rev.id,code(input.representationType,'Representation type',64),required(input.contentReference,'Content reference'),input.contentMediaType?.trim()||null,input.sourceFilename?.trim()||null,h.algorithm,h.value,c.actorPartyId,now()],e);await executeMutation('UPDATE information_containers SET aggregate_version = aggregate_version + 1, updated_at = ? WHERE id = ? AND tenant_id = ? AND aggregate_version = ?',[now(),current.id,c.tenantId,expected],e);const updated=await getContainer(c,current.id,e);await evidence(c,updated,'INFORMATION_REPRESENTATION_ADDED',current.status,current.status,{revisionNo:current.currentRevisionNo,representationType:input.representationType},e);});}

export async function submitInformationRevision(c:CommandContext,containerId:string,expected:number){assertPermission(c,'information.container.manage');return dbTransaction(async e=>{const current=await getContainer(c,containerId,e,true);if(current.aggregateVersion!==expected)throw new Error('This Information Container changed after you opened it.');const rev=await getRevision(c,current.id,current.currentRevisionNo,e,true);if(rev.lifecycleStatus!=='WORKING')throw new Error('Only a working revision can be submitted.');await executeMutation("UPDATE information_revisions SET lifecycle_status = 'REVIEW' WHERE id = ? AND lifecycle_status = 'WORKING'",[rev.id],e);await executeMutation("UPDATE information_containers SET status = 'IN_REVIEW', aggregate_version = aggregate_version + 1, updated_at = ? WHERE id = ? AND tenant_id = ? AND aggregate_version = ?",[now(),current.id,c.tenantId,expected],e);const updated=await getContainer(c,current.id,e);await evidence(c,updated,'INFORMATION_REVISION_SUBMITTED',current.status,'IN_REVIEW',{revisionNo:rev.revisionNo,revisionCode:rev.revisionCode},e);});}

export async function approveInformationRevision(c:CommandContext,containerId:string,expected:number,decisionId:string){assertPermission(c,'information.container.approve');return dbTransaction(async e=>{const current=await getContainer(c,containerId,e,true);if(current.aggregateVersion!==expected)throw new Error('This Information Container changed after you opened it.');const rev=await getRevision(c,current.id,current.currentRevisionNo,e,true);if(rev.lifecycleStatus!=='REVIEW')throw new Error('Only an in-review revision can be approved.');const decision=await assertWorkDecisionReference(c,{decisionId,decisionType:'INFORMATION_REVISION_REVIEW',subjectType:'INFORMATION_CONTAINER',subjectId:current.id,subjectVersion:String(rev.revisionNo),outcome:'APPROVED'},e);const ts=now();await executeMutation("UPDATE information_revisions SET lifecycle_status = 'APPROVED', approval_decision_id = ?, approved_at = ? WHERE id = ? AND lifecycle_status = 'REVIEW'",[decision.id,ts,rev.id],e);await executeMutation("UPDATE information_containers SET status = 'APPROVED', aggregate_version = aggregate_version + 1, updated_at = ? WHERE id = ? AND tenant_id = ? AND aggregate_version = ?",[ts,current.id,c.tenantId,expected],e);const updated=await getContainer(c,current.id,e);await evidence(c,updated,'INFORMATION_REVISION_APPROVED','IN_REVIEW','APPROVED',{revisionNo:rev.revisionNo,decisionId:decision.id},e);});}
