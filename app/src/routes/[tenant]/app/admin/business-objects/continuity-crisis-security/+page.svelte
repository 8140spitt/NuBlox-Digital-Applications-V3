<script lang="ts">
  import {
    ...
    continuityCrisisSecurityRelationships,
    continuityCrisisSecurityRules
  } from '$lib/data/continuity-crisis-security-model';

  let { data } = $props();
  const names = new Map(continuityCrisisSecurityModel.map((x) => [x.modelId, x.canonicalName]));
  const external = new Map([
    ['WORK-FOLLOW-UP-ACTION','Decision Action'],['WORK-DECISION','Decision'],
    ['KRC-COMMUNICATION-ITEM','Communication Item'],['IT-DISASTER-RECOVERY-PLAN','Disaster Recovery Plan'],
    ['CBO-SITE','Site'],['BE-SPACE','Space'],['CBO-PARTY','Party'],['CBO-ASSET','Asset'],
    ['RISK-ASSESSMENT','Risk Assessment']
  ]);
  const nameFor=(id:string)=>names.get(id)??external.get(id)??id;
</script>

<svelte:head><title>Continuity, Crisis & Security Semantics · NuBlox</title></svelte:head>

<div class="page">
  <header class="hero card">
    <div>
      <span class="eyebrow">Resilience and physical security</span>
      <h1>Business Continuity, Crisis & Physical Security</h1>
      <p>Impact analysis, recovery requirements, continuity planning, crisis response and physical-access/security controls around canonical enterprise, people, spatial and evidence identities.</p>
    </div>
    <div class="actions">
      ...
      <a href={'/'+data.tenantSlug+'/app/admin/business-objects/risk-compliance-audit'}>Risk, compliance & audit</a>
      <strong>{continuityCrisisSecurityModel.length} governed constructs</strong>
      <span>{continuityCrisisSecurityRelationships.length} relationships</span>
    </div>
  </header>

  <section class="card chain">
    <div><span>Impact</span><strong>BIA → Recovery Requirements</strong></div>
    <div><span>Continuity</span><strong>Strategy → Plan → Exercise</strong></div>
    <div><span>Crisis</span><strong>Event → Crisis → Decision / Action</strong></div>
    <div><span>Security</span><strong>Zone → Credential → Access Event</strong></div>
    <div><span>Incident</span><strong>Security Incident → Investigation / Action</strong></div>
  </section>

  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Semantic contracts</span><h2>Governed resilience and security constructs</h2></div><p>Continuity/security context surrounds shared enterprise truth rather than creating parallel people, site, asset, risk or workflow systems.</p></div>
    <div class="grid">
      {#each continuityCrisisSecurityModel as item,i}
        <details open={i<4}>
  ...
  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Relationship graph</span><h2>Impact-to-response-to-security provenance</h2></div></div>
    <div class="table"><table><thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Governance</th></tr></thead><tbody>
      {#each continuityCrisisSecurityRelationships as r}<tr><td><code>{r.id}</code></td><td>{nameFor(r.from)}</td><td>{r.predicate}</td><td>{nameFor(r.to)}</td><td>{r.governance}</td></tr>{/each}
    </tbody></table></div>
  </section>

  <section class="card content rules">
    <span class="eyebrow">Non-negotiable rules</span><h2>What NuBlox must preserve</h2>
    <ol>{#each continuityCrisisSecurityRules as x}<li>{x}</li>{/each}</ol>
  </section>
</div>
