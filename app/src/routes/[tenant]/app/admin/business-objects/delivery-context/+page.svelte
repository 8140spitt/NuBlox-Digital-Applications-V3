<script lang="ts">
  ...
  } from '$lib/data/delivery-context-model';

  let { data } = $props();
  const names = new Map(deliveryContextModel.map((item) => [item.modelId, item.canonicalName]));
</script>

<svelte:head><title>Delivery Context Semantics · NuBlox</title></svelte:head>

<div class="page">
  <header class="hero section-card">
    <div>
      <span class="eyebrow">Canonical delivery model</span>
      <h1>Delivery Context Semantics</h1>
      <p>Governed Portfolio, Programme, Project, stage, WBS, work-package, schedule, baseline, progress and resource-planning semantics. These structures are related deliberately without being collapsed into one hierarchy.</p>
    </div>
    <div class="hero-actions">
      ...
      <strong>{deliveryContextModel.length} delivery constructs</strong>
      <span>{deliveryContextRelationships.length} governed relationships</span>
    </div>
  </header>

  <section class="section-card boundary-grid">
    {#each deliveryStructureBoundaries as boundary}
      <article>
  ...
  <section class="section-card model">
    <div class="section-heading">
      <div><span class="eyebrow">Semantic contracts</span><h2>Governed delivery constructs</h2></div>
      <p>Open any construct to inspect its identity, scope, lifecycle and modelling boundaries.</p>
    </div>
    <div class="object-grid">
      {#each deliveryContextModel as item, index}
  ...
  <section class="section-card relationships">
    <div class="section-heading">
      <div><span class="eyebrow">Relationship graph</span><h2>How the delivery structures connect</h2></div>
      <p>Cardinality is logical guidance. Relationship records can carry effectivity, provenance, role and control metadata.</p>
    </div>
    <div class="table-wrap">
      <table>
        <thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Cardinality</th><th>Governance</th></tr></thead>
        <tbody>
          {#each deliveryContextRelationships as relationship}
            <tr>
