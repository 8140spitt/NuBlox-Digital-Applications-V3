<script lang="ts">
  ...
  let { data } = $props();
  const names = new Map(crmBusinessDevelopmentModel.map((item) => [item.modelId, item.canonicalName]));
...
<div class="page">
  <header class="hero section-card">
    <div>
      ...
      <p>Governed market, funnel, account-planning, interaction, onboarding and customer-case semantics built on the same canonical Party and Party Relationship identities.</p>
    </div>
  ...
  <section class="section-card model">
    <div class="section-heading">
      <div><span class="eyebrow">Semantic contracts</span><h2>Governed CRM constructs</h2></div>
      <p>Identity is preserved through the funnel: commercial objects link by provenance instead of one mutable record changing meaning from lead to project.</p>
    </div>
  ...
  <section class="section-card relationships">
    <div class="section-heading">
      <div><span class="eyebrow">Relationship graph</span><h2>How the commercial funnel connects to shared truth</h2></div>
      <p>Party/Organisation identity remains authoritative while Lead, Opportunity, Pursuit, decisions and customer cases retain their own independent business meaning.</p>
    </div>
