...
describe('asset operations semantic model', () => {
  it('is internally valid and relationship-complete', () => {
    ...
    expect(new Set(assetOperationsModel.map((entry) => entry.modelId)).size).toBe(assetOperationsModel.length);
  ...
  it('preserves System and Asset identity through commissioning and handover', () => {
    expect(assetOperationsRules.join(' ')).toContain('does not recreate System, Asset or Component identity');
  ...
  it('separates maintenance definition from execution', () => {
    expect(assetOperationsModel.find((entry) => entry.modelId === 'OPS-MAINTENANCE-PLAN')?.kind).toBe('plan');
    expect(assetOperationsModel.find((entry) => entry.modelId === 'OPS-TASK-TEMPLATE')?.kind).toBe('controlled-definition');
    expect(assetOperationsModel.find((entry) => entry.modelId === 'OPS-WORK-ORDER')?.kind).toBe('work');
  });

  it('keeps failure, defect and history semantically distinct', () => {
    expect(assetOperationsModel.find((entry) => entry.modelId === 'OPS-FAILURE')?.kind).toBe('event-evidence');
    expect(assetOperationsModel.find((entry) => entry.modelId === 'OPS-DEFECT')?.kind).toBe('case');
    expect(assetOperationsModel.find((entry) => entry.modelId === 'OPS-SERVICE-HISTORY')?.kind).toBe('projection');
    expect(assetOperationsRules.join(' ')).toContain('Failure is an event; Defect is a governed case');
  });

  it('keeps service intake, coordination, appointment and work separate', () => {
    for (const id of ['OPS-SERVICE-REQUEST', 'OPS-SERVICE-CASE', 'OPS-SERVICE-APPOINTMENT', 'OPS-WORK-ORDER']) {
