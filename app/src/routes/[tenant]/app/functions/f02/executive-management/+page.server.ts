...
import {
  completeExecutiveMeeting,
  conveneExecutiveMeeting,
  createExecutiveAction,
  createExecutiveMeeting,
  listExecutiveActionIds,
  listExecutiveAgenda,
  listExecutiveAttendees,
  listExecutiveDecisionIds,
...
function target(tenant: string, id?: string) {
  return `/${tenant}/app/functions/f02/executive-management${id ? '?meeting=' + encodeURIComponent(id) : ''}`;
}
  ...
  return fail(400, {
    message:
      error instanceof Error
        ? error.message
        : 'The Executive Governance command could not be completed.'
  });
}
...
export const load: PageServerLoad = async ({ params, url, locals }) => {
  ...
  const [attendeeRows, agendaRows, decisions, actions] = selected
    ? await Promise.all([
        listExecutiveAttendees(context, selected.id),
        listExecutiveAgenda(context, selected.id),
        listExecutiveDecisionIds(context, selected.id),
        listExecutiveActionIds(context, selected.id)
      ])
    : [[], [], [], []];
  return {
...
export const actions: Actions = {
  create: async ({ request, params, locals }) => {
    ...
    try {
      const id = await createExecutiveMeeting(
  ...
  attendance: async ({ request, params, locals }) => {
    const data = await request.formData(),
      id = text(data, 'meetingId');
    try {
      await recordExecutiveAttendance(
        await resolveRequestCommandContext(params.tenant, locals),
        id,
        text(data, 'partyId'),
  ...
  decision: async ({ request, params, locals }) => {
    const data = await request.formData(),
      id = text(data, 'meetingId');
    try {
      await recordExecutiveDecision(await resolveRequestCommandContext(params.tenant, locals), id, {
  ...
  action: async ({ request, params, locals }) => {
    const data = await request.formData(),
      id = text(data, 'meetingId');
    try {
      await createExecutiveAction(await resolveRequestCommandContext(params.tenant, locals), id, {
  ...
  complete: async ({ request, params, locals }) => {
    const data = await request.formData(),
      id = text(data, 'meetingId');
    try {
      await completeExecutiveMeeting(
        await resolveRequestCommandContext(params.tenant, locals),
        id,
        integer(data, 'aggregateVersion'),
        {
