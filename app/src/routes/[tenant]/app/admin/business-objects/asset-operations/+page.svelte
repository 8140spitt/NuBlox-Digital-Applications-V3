<script lang="ts">
  import {
    assetOperationsBoundaries,
    assetOperationsModel,
    assetOperationsRelationships,
    assetOperationsRules
  } from '$lib/data/asset-operations-model';

  let { data } = $props();
  const names = new Map(assetOperationsModel.map((item) => [item.modelId, item.canonicalName]));
  const externalNames = new Map([
    ['CBO-SYSTEM', 'System'],
    ['CBO-ASSET', 'Asset'],
    ['CBO-WARRANTY', 'Warranty'],
...
<svelte:head><title>Asset Operations Semantics · NuBlox</title></svelte:head>

<div class="page">
  <header class="hero section-card">
    <div>
      <span class="eyebrow">Whole-life asset model</span>
      <h1>Asset Operations, Commissioning & Service</h1>
      <p>Governed commissioning, handover, maintenance, service, reliability, warranty and operational evidence semantics built around one persistent System / Asset identity.</p>
    </div>
    <div class="hero-actions">
      ...
      <a href={`/${data.tenantSlug}/app/admin/business-objects/inventory-logistics`}>Inventory & logistics</a>
      <strong>{assetOperationsModel.length} governed constructs</strong>
      <span>{assetOperationsRelationships.length} semantic relationships</span>
    </div>
  </header>

  <section class="section-card boundary-grid">
    {#each assetOperationsBoundaries as boundary}
      <article>
  ...
  <section class="section-card model">
    <div class="section-heading">
      <div><span class="eyebrow">Semantic contracts</span><h2>Governed operational constructs</h2></div>
      <p>Each construct defines identity, scope, lifecycle and the boundary that prevents commissioning, FM and service activity from creating duplicate Asset truth.</p>
    </div>
    <div class="object-grid">
      {#each assetOperationsModel as item, index}
  ...
  <section class="section-card relationships">
    <div class="section-heading">
      <div><span class="eyebrow">Relationship graph</span><h2>How delivery becomes whole-life operations</h2></div>
      <p>These links preserve provenance and responsibility without turning Project closeout into an Asset re-creation event.</p>
    </div>
    <div class="table-wrap">
      <table>
        <thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Cardinality</th><th>Governance</th></tr></thead>
        <tbody>
          {#each assetOperationsRelationships as relationship}
            <tr>
