...
describe('sustainability, carbon, energy, circularity and social value model', () => {
  ...
  it('reuses operations utility consumption instead of a sustainability duplicate', () => {
    const decision = sustainabilityCarbonCanonicalization.find(
      (x) => x.candidateKey === 'BOF-20-010'
    );
  ...
  it('keeps embodied carbon as an assessment line rather than another Item master', () => {
    expect(
      sustainabilityCarbonCanonicalization.find((x) => x.candidateKey === 'BOF-20-007')?.decision
    ).toBe('CHILD');
  ...
  it('separates baseline, budget, target and assessment', () => {
    expect(sustainabilityCarbonModel.find((x) => x.modelId === 'SUS-CARBON-BASELINE')?.kind).toBe(
      'snapshot'
    );
    expect(sustainabilityCarbonModel.find((x) => x.modelId === 'SUS-CARBON-BUDGET')?.kind).toBe(
      'plan'
    );
    expect(sustainabilityCarbonModel.find((x) => x.modelId === 'SUS-CARBON-TARGET')?.kind).toBe(
      'target'
    );
    expect(sustainabilityCarbonModel.find((x) => x.modelId === 'SUS-CARBON-ASSESSMENT')?.kind).toBe(
      'projection'
    );
  });
  ...
  it('keeps social-value commitment, evidence and outcome separate', () => {
    expect(sustainabilityCarbonModel.find((x) => x.modelId === 'SUS-SOCIAL-COMMITMENT')?.kind).toBe(
      'commitment'
    );
    expect(sustainabilityCarbonModel.find((x) => x.modelId === 'SUS-SOCIAL-EVIDENCE')?.kind).toBe(
      'event-evidence'
    );
    expect(sustainabilityCarbonModel.find((x) => x.modelId === 'SUS-SOCIAL-OUTCOME')?.kind).toBe(
      'projection'
    );
  });
});
