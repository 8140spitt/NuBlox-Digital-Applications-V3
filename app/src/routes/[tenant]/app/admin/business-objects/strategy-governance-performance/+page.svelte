<script lang="ts">
  import {
    ...
    strategyGovernancePerformanceRelationships,
    strategyGovernancePerformanceRules
  } from '$lib/data/strategy-governance-performance-model';

  let { data } = $props();
  const names = new Map(strategyGovernancePerformanceModel.map((x) => [x.modelId, x.canonicalName]));
  const external = new Map([
    ['WORK-DECISION','Decision'],['WORK-FOLLOW-UP-ACTION','Decision Action'],
    ['AUTH-DELEGATED-AUTHORITY','Delegated Authority'],['CBO-INFORMATION-CONTAINER','Information Container'],
    ['DEL-PROGRAMME','Programme'],['CBO-PROJECT','Project'],['DEL-PORTFOLIO','Portfolio']
  ]);
  const nameFor=(id:string)=>names.get(id)??external.get(id)??id;
</script>

<svelte:head><title>Strategy & Governance Semantics · NuBlox</title></svelte:head>

<div class="page">
  <header class="hero card">
    <div>
      <span class="eyebrow">Enterprise strategy and governance</span>
      <h1>Strategy, Governance & Enterprise Performance</h1>
      <p>Strategy, plans, KPI truth and corporate governance built on shared decision, authority, information and evidence foundations.</p>
    </div>
    <div class="actions">
      ...
      <a href={'/'+data.tenantSlug+'/app/admin/business-objects/authority'}>Authority model</a>
      <strong>{strategyGovernancePerformanceModel.length} governed constructs</strong>
      <span>{strategyGovernancePerformanceRelationships.length} relationships</span>
    </div>
  </header>

  <section class="card chain">
    <div><span>Strategy</span><strong>Framework → Themes → Objectives</strong></div>
    <div><span>Execution</span><strong>Initiatives → Business Plan</strong></div>
    <div><span>Performance</span><strong>KPI → Target → Observation → Snapshot</strong></div>
    <div><span>Review</span><strong>Strategic Review → Governance</strong></div>
    <div><span>Authority</span><strong>Meeting → Decision → Action</strong></div>
  </section>

  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Semantic contracts</span><h2>Governed strategy and governance constructs</h2></div><p>Documents and workflows support the governance model; they never replace structured strategy, performance or decision truth.</p></div>
    <div class="grid">
      {#each strategyGovernancePerformanceModel as item,i}
        <details open={i<4}>
  ...
  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Relationship graph</span><h2>Strategy-to-decision provenance</h2></div></div>
    <div class="table"><table><thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Governance</th></tr></thead><tbody>
      {#each strategyGovernancePerformanceRelationships as r}<tr><td><code>{r.id}</code></td><td>{nameFor(r.from)}</td><td>{r.predicate}</td><td>{nameFor(r.to)}</td><td>{r.governance}</td></tr>{/each}
    </tbody></table></div>
  </section>

  <section class="card content rules">
    <span class="eyebrow">Non-negotiable rules</span><h2>What NuBlox must preserve</h2>
    <ol>{#each strategyGovernancePerformanceRules as x}<li>{x}</li>{/each}</ol>
  </section>
</div>
