...
<div class="master-page">
  <nav class="breadcrumb">
    <a href={`/${data.tenantSlug}/app/admin/master-data/parties`}>Party directory</a><span>›</span
    ><strong>Organisation identity stewardship</strong>
  </nav>

  <header class="hero section-card">
    <div>
      <span class="eyebrow">AGG-01-PARTY · Exceptional master stewardship</span>
      <h1>Organisation identity stewardship</h1>
      <p>
        Canonical organisations are originated by their home business workflows. This screen is
        reserved for resolving and correcting shared identity attributes; it is not an alternative
        customer or supplier onboarding route.
      </p>
    </div>
    <div class="authority">
      <strong>Stewardship authority</strong><span>{data.actorDisplayName}</span><small
        >Edit lease + recoverable draft + optimistic version control</small
      >
    </div>
  </header>
  ...
  {#if feedback?.message}
    <div class:conflict={feedback?.conflict} class="message" role="alert">
      ...
      {#if feedback?.holderDisplayName}<small>Current editor: {feedback?.holderDisplayName}</small
        >{/if}
    </div>
  {/if}
  ...
  <section class="metrics">
    <article class="section-card">
      <strong>{data.organisations.length}</strong><span>canonical organisations</span>
    </article>
    <article class="section-card">
    ...
    <aside class="register section-card">
      ...
      <div class="records">
        {#each data.organisations as organisation}
          ...
          >
            <span
              ><strong>{organisation.displayName}</strong><small
                >{organisation.registrationNumber || 'No registration number'}</small
              ></span
            >
        ...
        {:else}<p class="empty">
            No Organisation identities exist yet. Create them through their owning business
            workflow.
          </p>{/each}
      </div>
      <div class="creation-rule">
        <strong>Originate from the business</strong>
        <p>
          Clients: F07 · Suppliers: F09 · Employees/people: F15. Canonical identity resolution
          happens behind those workflows.
        </p>
      </div>
    </aside>
    ...
    <main class="detail">
      {#if data.selected}
        <section class="section-card editor">
          <div class="panel-heading">
            ...
            <div class="toolbar">
              ...
              <TaskContextButton
                ...
                workspaceFunctionId={data.origination?.originFunctionId?.startsWith('F')
                  ? data.origination.originFunctionId
                  : null}
              />
            </div>
          </div>
          ...
          <div class="record-meta">
            <span>Immutable ID <code>{data.selected.id}</code></span><span
              >Version <strong>{data.selected.version}</strong></span
          ...
          <section class="origin">
            ...
            <div class="origin-grid">
              <span
                ><small>Home function</small><strong
                  >{functionNames[data.origination?.originFunctionId ?? 'PLATFORM'] ??
                    data.origination?.originFunctionId ??
                    'Legacy'}</strong
                ></span
              >
              <span
                ><small>Origin object</small><strong
                  >{data.origination?.originObjectType ?? 'Legacy / platform'}</strong
                ></span
              >
              <span
                ><small>Steward</small><strong
                  >{functionNames[data.origination?.stewardFunctionId ?? 'PLATFORM'] ??
                    data.origination?.stewardFunctionId ??
                    'Platform'}</strong
                ></span
              >
            </div>
          ...
          {#if data.lease && !data.editing && !data.lease.holderIsCurrentActor}
            <div class="lease-banner locked">
              <strong>{data.lease.holderDisplayName} is currently editing this Organisation</strong
              ><span
                >You can continue to view it. The edit lease expires at {formatDate(
          ...
          {:else if data.lease && !data.editing && data.lease.holderIsCurrentActor}
            <div class="lease-banner">
              <strong>You have an active edit session</strong><span
                >Resume editing to restore your working draft.</span
              >
            </div>
          {/if}
          ...
          {#if data.editing && data.actorLease && data.workContext}
            <EditLeaseHeartbeat
              ...
              objectType="ORGANISATION"
              objectId={data.selected.id}
              leaseToken={data.actorLease.leaseToken}
            />
            ...
            {#if data.draft?.status === 'ACTIVE'}
              <div class:stale={!activeDraft()} class="draft-banner">
                <strong
                  >{activeDraft()
                    ? 'Recoverable draft restored'
                    : 'Draft retained against an older version'}</strong
                >
                <span
                  >{activeDraft()
                    ? 'Your unsaved fields were restored from the Task Bar work context.'
                    : 'The canonical Organisation changed after this draft was created. Review before copying any values forward.'}</span
                >
              </div>
            {/if}
            ...
            >
              ...
              <label class="full"
                >Legal name<input
                  name="legalName"
                  value={draftValue('legalName', data.selected.legalName)}
                  required
                /></label
              >
              <label
                >Trading name<input
                  name="tradingName"
                  value={draftValue('tradingName', data.selected.tradingName)}
                /></label
              >
              <label
                >Registration number<input
                  name="registrationNumber"
                  value={draftValue('registrationNumber', data.selected.registrationNumber)}
                /></label
              >
              <label
                >Tax identifier<input
                  name="taxIdentifier"
                  value={draftValue('taxIdentifier', data.selected.taxIdentifier)}
                /></label
              >
              <label
                >Country code<input
                  name="countryCode"
                  maxlength="2"
                  value={draftValue('countryCode', data.selected.countryCode)}
                /></label
              >
            ...
            <div class="editing-actions">
              <form method="POST" action="?/cancelEdit">
                <input type="hidden" name="id" value={data.selected.id} /><input
                  type="hidden"
                  name="editLeaseToken"
                  value={data.actorLease.leaseToken}
                /><button class="secondary" type="submit">Stop editing · keep draft</button>
              </form>
              <form method="POST" action="?/discardDraft">
                <input type="hidden" name="id" value={data.selected.id} /><input
                  ...
                  name="editLeaseToken"
                  value={data.actorLease.leaseToken}
                /><input type="hidden" name="workContextId" value={data.workContext.id} /><button
                  class="quiet"
                  type="submit">Discard draft & release</button
                >
              </form>
            </div>
          {:else}
            <dl class="read-grid">
              ...
              <div>
                ...
                <dd>{data.selected.taxIdentifier ?? '—'}</dd>
              </div>
              <div>
                <dt>Country</dt>
            ...
            {#if data.capabilities.canSteward && (!data.lease || data.lease.holderIsCurrentActor)}
              <form method="POST" action="?/beginEdit" class="begin-edit">
                <input type="hidden" name="id" value={data.selected.id} /><input
                  type="hidden"
                  name="version"
                  value={data.selected.version}
                /><input type="hidden" name="title" value={data.selected.displayName} /><button
                  type="submit"
                  >{data.lease?.holderIsCurrentActor
                    ? 'Resume stewardship edit'
                    : 'Begin stewardship edit'}</button
                >
              </form>
            {/if}
          {/if}
        ...
        {#if data.editing && data.actorLease && data.capabilities.canChangeStatus}
          <section class="section-card lifecycle">
            <div>
              <span class="eyebrow">Lifecycle stewardship</span>
              <h2>Organisation status</h2>
              <p>
                Lifecycle changes require the same active edit lease and canonical record version as
                attribute changes.
              </p>
            </div>
          ...
          <ol>
            {#each data.audit as event}
              <li>
                ...
                <span>{event.fromState || 'New'} → {event.toState || '—'}</span>{#if event.note}<p>
                    {event.note}
                  </p>{/if}
              </li>
      ...
      {:else}
        <section class="section-card welcome">
          <span class="eyebrow">Canonical identity</span>
          <h2>No Organisations yet</h2>
          <p>
            Create the first business relationship from its owning function. NuBlox will resolve or
            originate the shared Organisation identity there.
          </p>
        </section>
      {/if}
    </main>
