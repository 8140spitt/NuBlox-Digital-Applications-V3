<script lang="ts">
  import {
    ...
    peopleHcmRules
  } from '$lib/data/people-hcm-model';

  let { data } = $props();
  const names = new Map(peopleHcmModel.map((item) => [item.modelId, item.canonicalName]));
...
<svelte:head><title>People & HCM Semantics · NuBlox</title></svelte:head>

<div class="page">
  <header class="hero section-card">
    <div>
      <span class="eyebrow">Canonical workforce model</span>
      <h1>People, HCM, Competence, Time, Payroll & Expenses</h1>
      <p>One Person identity across recruitment, employment, position, competence, learning, scheduling, time, payroll and offboarding—with each business context retaining its own governed meaning.</p>
    </div>
    <div class="hero-actions">
      ...
      <strong>{peopleHcmModel.length} governed constructs</strong>
      <span>{peopleHcmRelationships.length} semantic relationships</span>
    </div>
  </header>

  <section class="section-card boundary-grid">
    {#each peopleHcmBoundaries as boundary}
      <article>
  ...
  <section class="section-card model">
    <div class="section-heading">
      <div><span class="eyebrow">Semantic contracts</span><h2>Governed workforce constructs</h2></div>
      <p>Identity, organisation structure, competence evidence, work time and pay consequences remain connected without collapsing into one employee record.</p>
    </div>
    <div class="object-grid">
      {#each peopleHcmModel as item, index}
  ...
  <section class="section-card relationships">
    <div class="section-heading">
      <div><span class="eyebrow">Relationship graph</span><h2>How workforce truth connects</h2></div>
      <p>Employment, organisational structure, competence, time and financial consequences reference the same worker identity while preserving domain authority.</p>
    </div>
    <div class="table-wrap">
      <table>
        <thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Cardinality</th><th>Governance</th></tr></thead>
        <tbody>
          {#each peopleHcmRelationships as relationship}
            <tr>
  ...
  <section class="section-card rules">
    ...
    <ol>{#each peopleHcmRules as rule}<li>{rule}</li>{/each}</ol>
  </section>
</div>
