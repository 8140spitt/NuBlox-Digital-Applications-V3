...
import {
  ...
  qhseAssuranceRules,
  validateQhseAssuranceModel
} from './qhse-assurance-model';
...
describe('quality, HSE, environment and assurance semantic model', () => {
  it('is internally valid and covers every BOF-13 candidate', () => {
    ...
    for (let i = 1; i <= 37; i += 1)
      expect(decided.has(`BOF-13-${String(i).padStart(3, '0')}`)).toBe(true);
  ...
  it('keeps NCR, Defect and CAPA distinct while normalizing snag/actions', () => {
    ...
    expect(
      qhseAssuranceCanonicalization.find((x) => x.candidateKey === 'BOF-13-009')?.targetCandidateKey
    ).toBe('BOF-13-008');
    expect(
      qhseAssuranceCanonicalization.find((x) => x.candidateKey === 'BOF-13-011')?.targetCandidateKey
    ).toBe('BOF-13-010');
  });

  it('reuses shared enterprise risk, audit and compliance semantics', () => {
    expect(
      qhseAssuranceCanonicalization.find((x) => x.candidateKey === 'BOF-13-018')?.targetCandidateKey
    ).toBe('BOF-21-003');
    expect(
      qhseAssuranceCanonicalization.find((x) => x.candidateKey === 'BOF-13-013')?.targetCandidateKey
    ).toBe('BOF-21-012');
    expect(
      qhseAssuranceCanonicalization.find((x) => x.candidateKey === 'BOF-13-032')?.targetCandidateKey
    ).toBe('BOF-21-006');
  });
  ...
  it('treats compliance register as projection and incident specializations consistently', () => {
    expect(
      qhseAssuranceCanonicalization.find((x) => x.candidateKey === 'BOF-13-033')?.decision
    ).toBe('PROJECTION');
    expect(
      qhseAssuranceCanonicalization.find((x) => x.candidateKey === 'BOF-13-027')?.targetCandidateKey
    ).toBe('BOF-13-028');
    expect(
      qhseAssuranceCanonicalization.find((x) => x.candidateKey === 'BOF-13-036')?.targetCandidateKey
    ).toBe('BOF-13-028');
  });
});
