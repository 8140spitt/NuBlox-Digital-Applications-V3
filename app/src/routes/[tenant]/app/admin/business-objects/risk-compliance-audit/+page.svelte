<script lang="ts">
  import {
    ...
    riskComplianceAuditRelationships,
    riskComplianceAuditRules
  } from '$lib/data/risk-compliance-audit-model';

  let { data } = $props();
  const names = new Map(riskComplianceAuditModel.map((x) => [x.modelId, x.canonicalName]));
...
<svelte:head><title>Risk, Compliance & Audit Semantics · NuBlox</title></svelte:head>

<div class="page">
  <header class="hero card">
    <div>
      <span class="eyebrow">Enterprise assurance backbone</span>
      <h1>Risk, Compliance, Internal Control & Audit</h1>
      <p>Shared risk identity, assessment evidence, obligations, requirements, controls, assurance planning, audit execution and remediation—reusable across every NuBlox workspace.</p>
    </div>
    <div class="actions">
      ...
      <strong>{riskComplianceAuditModel.length} governed constructs</strong>
      <span>{riskComplianceAuditRelationships.length} relationships</span>
    </div>
  </header>

  <section class="card chain">
    <div><span>Risk</span><strong>Framework → Risk → Assessment → Treatment</strong></div>
    <div><span>Compliance</span><strong>Obligation → Requirement → Evidence → Assessment</strong></div>
    <div><span>Control</span><strong>Internal Control → Control Test</strong></div>
    <div><span>Audit</span><strong>Assurance Plan → Engagement → Finding</strong></div>
    <div><span>Response</span><strong>Remediation / Integrity Case</strong></div>
  </section>

  <section class="card content">
    <div class="heading">
      <div><span class="eyebrow">Semantic contracts</span><h2>Governed enterprise-assurance constructs</h2></div>
      <p>Persistent definitions, plans, cases and immutable evidence are connected without collapsing into a generic GRC record.</p>
    </div>
    <div class="grid">
      {#each riskComplianceAuditModel as item, i}
        <details open={i < 4}>
  ...
  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Relationship graph</span><h2>Risk-to-assurance provenance</h2></div></div>
    <div class="table">
      <table>
        <thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Governance</th></tr></thead>
        <tbody>
          {#each riskComplianceAuditRelationships as r}
  ...
  <section class="card content rules">
    ...
    <ol>{#each riskComplianceAuditRules as x}<li>{x}</li>{/each}</ol>
  </section>
</div>
