<script lang="ts">
  ...
  let { data } = $props();
  const names = new Map(
    referenceConfigurationModel.map((item) => [item.modelId, item.canonicalName])
  );
...
<div class="page">
  <header class="hero section-card">
    <div>
      ...
      <p>
        Governed reference data, classifications, jurisdiction, lifecycle/workflow definitions and
        policy configuration reused across all 29 tenant workspaces.
      </p>
    </div>
    <div class="hero-actions">
      <a href={`/${data.tenantSlug}/app/admin/business-objects`}>← Canonicalization workbench</a>
      <a href={`/${data.tenantSlug}/app/admin/business-objects/shared-work-evidence`}
        >Shared work & evidence</a
      >
  ...
  <section class="section-card model">
    <div class="section-heading">
      <div>
        ...
        <h2>Governed reference and configuration constructs</h2>
      </div>
      <p>
        Each construct defines stable meaning and policy while keeping runtime business state on the
        canonical object that actually owns it.
      </p>
    </div>
  ...
  <section class="section-card relationships">
    <div class="section-heading">
      <div>
        ...
        <h2>How configuration controls without owning truth</h2>
      </div>
      <p>
        Runtime objects reference exact definitions, releases or rules so historical meaning
  ...
  <section class="section-card rules">
    ...
    <ol>
      {#each referenceConfigurationRules as rule}<li>{rule}</li>{/each}
    </ol>
  </section>
</div>
...
<style>
  ...
  .hero {
    ...
    align-items: center;
    padding: 20px;
    background: linear-gradient(120deg, #fbfdff, #f2f7f5 65%, #fff);
  }
  .eyebrow {
    color: var(--blue-700);
    font-size: 10px;
