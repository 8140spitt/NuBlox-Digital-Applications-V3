...
export const externalBenchmarkRegister: ExternalBenchmarkDefinition[] = [
  {
    ...
    provider: 'SAP',
    product:
      'SAP Cloud ERP / S/4HANA + Ariba + SuccessFactors + enterprise planning/asset capabilities',
    ...
    notes:
      'Primary enterprise completeness benchmark. The 64-line legacy SAP capability register is retained under docs/benchmarks as provenance and must be remapped to V3 workspaces/canonical objects rather than copied as module boundaries.'
  },
  {
    id: 'ORACLE-CLOUD-CX',
    provider: 'Oracle',
    ...
    challengeFocus: [
      'Primavera planning/control',
      'Unifier capital programme controls',
      'Aconex information exchange',
      'Textura payment management',
      'ERP continuity'
    ],
    notes:
      'Challenges project/programme control, capital delivery, CDE collaboration, payment and enterprise back-office continuity.'
  },
  {
    id: 'MICROSOFT-D365',
    provider: 'Microsoft',
    ...
    challengeFocus: [
      'field-to-project-finance',
      'record-to-report',
      'source-to-pay',
      'sales/service continuity',
      'Dataverse/Power Platform extensibility'
    ],
    notes:
      'Strong benchmark for connected operational and financial processes across field work, project operations and finance.'
  },
  {
    id: 'IFS-CLOUD',
    provider: 'IFS',
    ...
    notes:
      'Important benchmark for asset-intensive and service-centric operating models where ERP, EAM and FSM are integrated.'
  },
  {
    id: 'WORKDAY',
    provider: 'Workday',
    ...
    notes:
      'Specialist enterprise benchmark for people, finance and planning on a unified cloud data model.'
  },
  {
    id: 'SALESFORCE',
    provider: 'Salesforce',
    ...
    notes:
      'CRM/customer-service depth benchmark; not an authority for NuBlox enterprise or construction object boundaries.'
  },
  {
    id: 'SERVICENOW',
    provider: 'ServiceNow',
    ...
    notes:
      'Benchmark for enterprise workflow, request/case management, IT operations and cross-cutting controls.'
  },
  {
    id: 'DILIGENT-ONE',
    provider: 'Diligent',
    ...
    notes:
      'Specialist governance benchmark for board, corporate-secretariat and GRC evidence that broad ERP suites often underrepresent.'
  },
  {
    id: 'PROCORE',
    provider: 'Procore',
  ...
  },
  {
    ...
    challengeFocus: [
      ...
      'data governance',
      'multidiscipline infrastructure information',
      'digital twin continuity'
    ],
    notes:
      'Key engineering/infrastructure benchmark for controlled design information and digital-twin continuity.'
  },
  {
    ...
    challengeFocus: [
      ...
      'office-field integration'
    ],
    notes:
      'Construction ERP and project-operations benchmark linking commercial back office and field delivery.'
  },
  {
    id: 'IBM-MAXIMO',
    provider: 'IBM',
  ...
  },
  {
    ...
    notes:
      'Deep semantic benchmark for lifecycle/configuration governance; never automatic NuBlox schema authority.'
  },
  {
    ...
    challengeFocus: [
      ...
      'workflow',
      'digital thread',
      'manufacturing/service continuity'
    ],
    notes:
      'Second PLM benchmark to prevent Windchill-specific bias and challenge product/configuration semantics independently.'
  },
  {
    ...
    notes:
      'Important architecture/engineering consultancy benchmark connecting pursuit, people, projects and financial performance.'
  },
  {
    id: 'ASITE-CDE',
    provider: 'Asite',
    ...
    challengeFocus: [
      'ISO-style CDE workflows',
      'information delivery',
      'BIM coordination',
      'supply-chain marketplace',
      'project data continuity'
    ],
    notes:
      'Construction information-management benchmark with strong CDE and BIM collaboration focus.'
  },
  {
    id: 'THINKPROJECT',
    ...
    challengeFocus: [
      ...
      'NEC-style event workflows',
      'CDE',
      'field/document management',
      'built-asset work management'
    ],
    notes:
      'Built-environment lifecycle benchmark spanning project information, contract administration and operational asset work.'
  },
  {
    id: 'HEXAGON-ECOSYS',
    provider: 'Hexagon',
  ...
  },
  {
    ...
    notes:
      'Geospatial/infrastructure benchmark so NuBlox treats location, networks and field evidence as first-class semantics rather than map attachments.'
  },
  {
    id: 'PLANON-IWMS',
    provider: 'Planon',
    ...
    studyState: 'challenged',
    officialSource:
      'https://planonsoftware.com/uk/software/iwms/space-workplace-services-management/',
    challengeFocus: [
      'real estate',
      'lease/space',
      'workplace services',
    ...
    notes:
      'Specialist FM/property benchmark across workplace, space, facilities and property operations.'
  },
  {
    ...
    notes:
      'Important construction-finance benchmark, particularly for contractor job cost/WIP and UK operational accounting.'
  },
  {
    id: 'CAUSEWAY',
    ...
    notes:
      'UK construction-specialist challenge benchmark for contractor commercial, supply-chain and infrastructure workflows.'
  }
];

const allWorkspaceIds = new Set(enterpriseFunctions.map((fn) => fn.id));
const benchmarkWorkspaceCoverage = new Set(
  externalBenchmarkRegister.flatMap((entry) => entry.workspaces)
);

export const marketBenchmarkSummary = {
  benchmarkCount: externalBenchmarkRegister.length,
  mandatorySuiteCount: externalBenchmarkRegister.filter((entry) => entry.tier === 'mandatory-suite')
    .length,
  specialistDepthCount: externalBenchmarkRegister.filter(
    (entry) => entry.tier === 'specialist-depth'
  ).length,
  challengedBenchmarkCount: externalBenchmarkRegister.filter(
    (entry) => entry.studyState === 'challenged'
  ).length,
  inProgressBenchmarkCount: externalBenchmarkRegister.filter(
    (entry) => entry.studyState === 'in-progress'
  ).length,
  registeredBenchmarkCount: externalBenchmarkRegister.filter(
    (entry) => entry.studyState === 'registered'
  ).length,
...
export function validateExternalBenchmarkRegister() {
  if (
    new Set(externalBenchmarkRegister.map((entry) => entry.id)).size !==
    externalBenchmarkRegister.length
  )
    return false;
  if (!externalBenchmarkRegister.every((entry) => entry.officialSource.startsWith('https://')))
    return false;
  if (
    !externalBenchmarkRegister.every(
      (entry) => entry.workspaces.length > 0 && entry.challengeFocus.length > 0
    )
  )
    return false;
  if (
    !externalBenchmarkRegister.every((entry) =>
      entry.workspaces.every((workspace) => allWorkspaceIds.has(workspace))
    )
  )
    return false;
  ...
  if (
    marketBenchmarkSummary.standardsChallengedCount !==
    marketBenchmarkSummary.standardsChallengeCount
  )
    return false;
  if (marketBenchmarkSummary.rejectedVendorPatternOpenCount !== 0) return false;
  if (
    marketBenchmarkSummary.rejectedVendorPatternRecordedCount !==
    marketBenchmarkSummary.rejectedVendorPatternCount
  )
    return false;
  return true;
}
