<script lang="ts">
  import {
    referenceConfigurationBoundaries,
    referenceConfigurationModel,
    referenceConfigurationRelationships,
    referenceConfigurationRules
  } from '$lib/data/reference-configuration-model';

  let { data } = $props();
  const names = new Map(referenceConfigurationModel.map((item) => [item.modelId, item.canonicalName]));
  const externalNames = new Map([
    ['FIN-EXCHANGE-RATE', 'Exchange Rate'],
    ['CBO-CONTRACT', 'Contract'],
    ['WORK-WORKFLOW-INSTANCE', 'Workflow Instance'],
    ['DEL-STAGE-ASSIGNMENT', 'Delivery Stage Assignment'],
    ['CBO-INFORMATION-CONTAINER', 'Information Container'],
    ['AUTH-ROLE-ASSIGNMENT', 'Role Assignment'],
...
<svelte:head><title>Reference & Configuration Semantics · NuBlox</title></svelte:head>

<div class="page">
  <header class="hero section-card">
    <div>
      <span class="eyebrow">Shared configuration foundation</span>
      <h1>Reference, Classification & Policy</h1>
      <p>Governed reference data, classifications, jurisdiction, lifecycle/workflow definitions and policy configuration reused across all 29 tenant workspaces.</p>
    </div>
    <div class="hero-actions">
      <a href={`/${data.tenantSlug}/app/admin/business-objects`}>← Canonicalization workbench</a>
      <a href={`/${data.tenantSlug}/app/admin/business-objects/shared-work-evidence`}>Shared work & evidence</a>
      <strong>{referenceConfigurationModel.length} governed constructs</strong>
      <span>{referenceConfigurationRelationships.length} semantic relationships</span>
    </div>
  </header>

  <section class="section-card boundary-grid">
    {#each referenceConfigurationBoundaries as boundary}
      <article>
  ...
  <section class="section-card model">
    <div class="section-heading">
      <div><span class="eyebrow">Semantic contracts</span><h2>Governed reference and configuration constructs</h2></div>
      <p>Each construct defines stable meaning and policy while keeping runtime business state on the canonical object that actually owns it.</p>
    </div>
    <div class="object-grid">
      {#each referenceConfigurationModel as item, index}
  ...
  <section class="section-card relationships">
    <div class="section-heading">
      <div><span class="eyebrow">Relationship graph</span><h2>How configuration controls without owning truth</h2></div>
      <p>Runtime objects reference exact definitions, releases or rules so historical meaning survives configuration change.</p>
    </div>
    <div class="table-wrap">
      <table>
        <thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Cardinality</th><th>Governance</th></tr></thead>
        <tbody>
          {#each referenceConfigurationRelationships as relationship}
            <tr>
