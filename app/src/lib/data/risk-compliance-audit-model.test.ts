...
describe('enterprise risk, compliance, internal control and audit model', () => {
  ...
  it('separates Risk identity from assessment evidence and treatment planning', () => {
    expect(riskComplianceAuditModel.find((x) => x.modelId === 'RISK-ENTERPRISE-RISK')?.kind).toBe(
      'risk'
    );
    expect(riskComplianceAuditModel.find((x) => x.modelId === 'RISK-ASSESSMENT')?.kind).toBe(
      'event-evidence'
    );
    expect(riskComplianceAuditModel.find((x) => x.modelId === 'RISK-TREATMENT-PLAN')?.kind).toBe(
      'plan'
    );
  ...
  it('separates regulatory obligation, requirement, assessment and evidence', () => {
    expect(
      riskComplianceAuditModel.find((x) => x.modelId === 'COMP-REGULATORY-OBLIGATION')?.kind
    ).toBe('obligation');
    expect(riskComplianceAuditModel.find((x) => x.modelId === 'COMP-REQUIREMENT')?.kind).toBe(
      'controlled-requirement'
    );
    expect(riskComplianceAuditModel.find((x) => x.modelId === 'COMP-ASSESSMENT')?.kind).toBe(
      'event-evidence'
    );
    expect(riskComplianceAuditModel.find((x) => x.modelId === 'COMP-EVIDENCE')?.kind).toBe(
      'event-evidence'
    );
  });
  ...
  it('keeps Internal Control and Control Test distinct', () => {
    expect(
      riskComplianceAuditCanonicalization.find((x) => x.candidateKey === 'BOF-21-008')?.decision
    ).toBe('VALIDATE_OBJECT');
    expect(
      riskComplianceAuditCanonicalization.find((x) => x.candidateKey === 'BOF-21-009')?.decision
    ).toBe('EVENT_EVIDENCE');
  });

  it('normalizes Audit Plan onto Assurance Plan while preserving Audit Engagement', () => {
    const auditPlan = riskComplianceAuditCanonicalization.find(
      (x) => x.candidateKey === 'BOF-21-011'
    );
    expect(auditPlan?.decision).toBe('MERGE');
    expect(auditPlan?.targetCandidateKey).toBe('BOF-21-010');
    expect(
      riskComplianceAuditCanonicalization.find((x) => x.candidateKey === 'BOF-21-012')
        ?.proposedCanonicalName
    ).toBe('Audit Engagement');
  });

  it('normalizes Fraud and Conduct into one Integrity Case pattern', () => {
    expect(
      riskComplianceAuditCanonicalization.find((x) => x.candidateKey === 'BOF-21-015')
        ?.proposedCanonicalName
    ).toBe('Integrity Case');
    const conduct = riskComplianceAuditCanonicalization.find(
      (x) => x.candidateKey === 'BOF-21-016'
    );
