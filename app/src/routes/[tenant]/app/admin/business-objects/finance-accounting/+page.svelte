<script lang="ts">
  import {
    financeAccountingBoundaries,
    financeAccountingModel,
    financeAccountingRelationships,
    financeAccountingRules
  } from '$lib/data/finance-accounting-model';

  let { data } = $props();
  const names = new Map(financeAccountingModel.map((item) => [item.modelId, item.canonicalName]));
  const externalNames = new Map([
    ['CBO-LEGAL-ENTITY', 'Legal Entity'],
    ['CBO-PARTY', 'Party'],
    ['CBO-PROJECT', 'Project'],
    ['DEL-WBS-ELEMENT', 'WBS Element'],
    ['CBO-CONTRACT', 'Contract'],
    ['CBO-ASSET', 'Asset'],
    ['PROC-PURCHASE-ORDER', 'Purchase Order']
  ]);
  const nameFor = (id: string) => names.get(id) ?? externalNames.get(id) ?? id;
</script>

<svelte:head><title>Finance & Accounting Semantics · NuBlox</title></svelte:head>

<div class="page">
  <header class="hero section-card">
    <div>
      <span class="eyebrow">Canonical finance model</span>
      <h1>Finance, Accounting & Treasury</h1>
      <p>Governed ledger, AP/AR, settlement, tax, fixed-asset accounting, treasury and project-financial-control semantics. Finance records financial effects while reusing the same enterprise business identities.</p>
    </div>
    <div class="hero-actions">
      ...
      <a href={`/${data.tenantSlug}/app/admin/business-objects/asset-operations`}>Asset operations</a>
      <strong>{financeAccountingModel.length} governed constructs</strong>
      <span>{financeAccountingRelationships.length} semantic relationships</span>
    </div>
  </header>

  <section class="section-card boundary-grid">
    {#each financeAccountingBoundaries as boundary}
      <article>
  ...
  <section class="section-card model">
    <div class="section-heading">
      <div><span class="eyebrow">Semantic contracts</span><h2>Governed financial constructs</h2></div>
      <p>Each construct defines identity, scope and lifecycle while preserving the boundary between business truth, accounting evidence and derived financial positions.</p>
    </div>
    <div class="object-grid">
      {#each financeAccountingModel as item, index}
  ...
  <section class="section-card relationships">
    <div class="section-heading">
      <div><span class="eyebrow">Relationship graph</span><h2>How finance references enterprise truth</h2></div>
      <p>Posting and reporting dimensions point back to canonical legal, commercial, project and asset identities instead of reproducing them inside Finance.</p>
    </div>
    <div class="table-wrap">
      <table>
        <thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Cardinality</th><th>Governance</th></tr></thead>
        <tbody>
          {#each financeAccountingRelationships as relationship}
            <tr>
