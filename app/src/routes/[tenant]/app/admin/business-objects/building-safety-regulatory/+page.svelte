<script lang="ts">
  import { buildingSafetyRegulatoryModel, buildingSafetyRegulatoryRelationships, buildingSafetyRegulatoryRules } from '$lib/data/building-safety-regulatory-model';
  let { data } = $props();
  const names = new Map(buildingSafetyRegulatoryModel.map((x) => [x.modelId, x.canonicalName]));
  const external = new Map([
    ['CBO-PARTY','Party'],['BE-BUILDING','Building'],['REF-REGULATORY-REGIME','Regulatory Regime'],
    ['HCM-PERSON-COMPETENCE','Person Competence'],['HCM-CREDENTIAL','Person Credential'],
    ['CBO-INFORMATION-CONTAINER','Information Container'],['WORK-DECISION','Decision'],
...
<div class="page">
  <header class="hero card">
    <div><span class="eyebrow">Statutory assurance architecture</span><h1>Building Safety, Regulatory Control & Statutory Assurance</h1><p>Dutyholder, competence, regulator-case, controlled-change, inspection, decision, completion and golden-thread semantics built around the same canonical Building, Person and information identities.</p></div>
    <div class="actions"><a href={`/${data.tenantSlug}/app/admin/business-objects`}>← Canonicalization workbench</a><a href={`/${data.tenantSlug}/app/admin/business-objects/coverage-audit`}>Coverage audit</a><a href={`/${data.tenantSlug}/app/admin/business-objects/qhse-assurance`}>QHSE & assurance</a><strong>{buildingSafetyRegulatoryModel.length} governed constructs</strong><span>{buildingSafetyRegulatoryRelationships.length} relationships</span></div>
  </header>
  <section class="card chain">
    ...
    <div><span>Change & assurance</span><strong>Controlled Change → Inspection → Finding</strong></div>
  ...
  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Semantic contracts</span><h2>Governed regulatory constructs</h2></div><p>The regulator-facing record stays traceable without becoming a second project, building or document system.</p></div>
    <div class="grid">{#each buildingSafetyRegulatoryModel as item,i}<details open={i<4}><summary><span><code>{item.modelId}</code><strong>{item.canonicalName}</strong></span><small>{item.kind}</small></summary><div class="body"><p>{item.definition}</p><dl><div><dt>Candidate</dt><dd>{item.candidateKeys.join(', ')}</dd></div><div><dt>Identity</dt><dd>{item.identityRule}</dd></div><div><dt>Lifecycle</dt><dd>{item.lifecycle.join(' · ')}</dd></div><div><dt>Governance</dt><dd><ul>{#each item.governance as x}<li>{x}</li>{/each}</ul></dd></div></dl></div></details>{/each}</div>
  </section>
  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Relationship graph</span><h2>Regulatory provenance</h2></div></div>
    <div class="table"><table><thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Governance</th></tr></thead><tbody>{#each buildingSafetyRegulatoryRelationships as r}<tr><td><code>{r.id}</code></td><td>{nameFor(r.from)}</td><td>{r.predicate}</td><td>{nameFor(r.to)}</td><td>{r.governance}</td></tr>{/each}</tbody></table></div>
  </section>
  <section class="card content rules"><span class="eyebrow">Non-negotiable rules</span><h2>What NuBlox must preserve</h2><ol>{#each buildingSafetyRegulatoryRules as x}<li>{x}</li>{/each}</ol></section>
</div>
<style>
