  ...
  <div class="workspace">
    <aside class="section-card register">
      <div class="section-title">
        <div>
          <span class="eyebrow">Policy register</span>
          <h2>{data.policies.length} policies</h2>
        </div>
      </div>
      ...
      <nav class="policy-list">
        {#each data.policies as item}
          <a class:active={data.selected?.id === item.id} href={href(item.id)}>
            <span class="row-head"
              ><strong>{item.policyRef}</strong><small>{item.status}</small></span
            >
          ...
          <form method="POST" action="?/create">
            ...
            <label
              ...
              <textarea
                ...
                rows="3"
                required
                placeholder="Who and what this policy applies to"></textarea>
            </label>
            ...
            {#if data.capabilities.canReadParties}
              <label
                ...
                <select name="ownerPartyId">
                  ...
                  {#each data.parties as party}<option value={party.id}>{party.displayName}</option
                    >{/each}
                </select>
              </label>
            {/if}
            ...
            {#if data.capabilities.canReadBodies}
              <label
                ...
                <select name="governanceBodyId">
                  ...
                  {#each data.bodies as body}<option value={body.id}
                      >{body.bodyRef} · {body.name}</option
                    >{/each}
                </select>
              </label>
            {/if}
            ...
            <label class="check"
              ><input type="checkbox" name="attestationRequired" /><span
                >Require attestation for this revision</span
              ></label
            >
    ...
    <main class="content">
      {#if data.selected}
        <section class="section-card policy-card">
          ...
          <div class="facts">
            ...
            <article>
              <small>Current revision</small><strong>{data.selected.currentRevisionCode}</strong>
            </article>
            <article><small>Owner</small><strong>{data.selected.ownerDisplayName}</strong></article>
            <article>
              <small>Governance body</small><strong
                >{data.selected.governanceBodyName ?? '—'}</strong
              >
            </article>
            <article>
              <small>Effective from</small><strong>{date(data.selected.effectiveFrom)}</strong>
            </article>
            <article>
              <small>Review due</small><strong>{date(data.selected.reviewDueAt)}</strong>
            </article>
            <article>
              <small>Attestation</small><strong>{yesNo(data.selected.attestationRequired)}</strong>
            </article>
            <article>
              <small>Aggregate</small><strong>v{data.selected.aggregateVersion}</strong>
            </article>
          </div>
        ...
        <div class="lower-grid">
          <section class="section-card representations">
            <div class="section-title">
              <div>
                <span class="eyebrow">Controlled representations</span>
                <h3>{data.representations.length} files / renditions</h3>
              </div>
            ...
            <div class="representation-list">
              {#each data.representations as item}
                <article>
                  <div>
                    <strong>{item.sourceFilename ?? item.representationType}</strong><small
                      >{item.contentMediaType ?? item.representationType}</small
                    >
                  </div>
              ...
              <details>
                ...
                <form method="POST" action="?/representation">
                  ...
                  <div class="pair">
                    ...
                    <label
                      >Media type<input name="contentMediaType" value="application/pdf" /></label
                    >
                  </div>
                  <label
                    >Content reference<input
                      ...
                      required
                      placeholder="s3://… or governed repository reference"
                    /></label
                  >
                  <label>Source filename<input name="sourceFilename" /></label>
                  <div class="pair">
                    <label
                      >Hash algorithm<select name="hashAlgorithm"
                        ><option value="SHA256">SHA-256</option><option value="SHA512"
                          >SHA-512</option
                        ></select
                      ></label
                    >
          ...
          <section class="section-card history">
            <div class="section-title">
              <div>
                <span class="eyebrow">Revision history</span>
            ...
            <div class="revision-list">
              {#each data.revisions as revision}
                <article>
                  <div class="row-head">
                    <strong>{revision.revisionCode}</strong><span>{revision.lifecycleStatus}</span>
                  </div>
                  <p>{revision.title}</p>
                  <small
                    >{revision.ownerDisplayName} · effective {date(revision.effectiveFrom)}</small
                  >
                  {#if revision.approvalDecisionId}<small
                      >Decision {revision.approvalDecisionId.slice(0, 8)}</small
                    >{/if}
                </article>
              {/each}
            </div>
          ...
          <section class="section-card successor">
            <details>
              ...
              <form method="POST" action="?/successor">
                ...
                <div class="pair">
                  <label
                    >Revision code<input name="revisionCode" required placeholder="P02" /></label
                  >
                  <label>Title<input name="title" value={data.selected.title} /></label>
                </div>
                <label
                  >Applicability<textarea name="applicabilitySummary" rows="3"
                    >{data.selected.applicabilitySummary}</textarea
                  ></label
                >
                <div class="pair">
                ...
                {#if data.capabilities.canReadParties}
                  <label
                    >Owner
                    <select name="ownerPartyId">
                      {#each data.parties as party}<option
                          value={party.id}
                          selected={party.id === data.selected.ownerPartyId}
                ...
                {#if data.capabilities.canReadBodies}
                  <label
                    ...
                    <select name="governanceBodyId">
                      <option value="">No body assigned</option>
                      {#each data.bodies as body}<option
                          value={body.id}
                          selected={body.id === data.selected.governanceBodyId}
                          >{body.bodyRef} · {body.name}</option
                        >{/each}
                    </select>
                  </label>
                ...
                <label class="check"
                  ><input
                    ...
                    checked={Boolean(data.selected.attestationRequired)}
                  /><span>Require attestation</span></label
                >
      ...
      {:else}
        <section class="section-card empty-state">
          ...
          <p>
            The Policy register will retain ownership, applicability, effectivity, review evidence
            and controlled revision history.
          </p>
        </section>
      {/if}
    </main>
...
<style>
  ...
  .representation-list article > div {
    display: flex;
    justify-content: space-between;
    gap: 8px;
  }
  .representation-list small,
  .revision-list small {
    font-size: 8px;
    color: #798b97;
  }
  .representation-list code {
    font-size: 8px;
