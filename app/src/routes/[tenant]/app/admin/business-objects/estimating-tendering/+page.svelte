<script lang="ts">
  import {
    estimatingTenderingBoundaries,
    estimatingTenderingModel,
    estimatingTenderingRelationships,
    estimatingTenderingRules
  } from '$lib/data/estimating-tendering-model';

  let { data } = $props();
  const names = new Map(estimatingTenderingModel.map((item) => [item.modelId, item.canonicalName]));
  const externalNames = new Map([
    ['CRM-OPPORTUNITY', 'Opportunity'],
    ['PROC-SOURCING-REQUEST', 'Sourcing Request'],
    ['PROC-SOURCING-RESPONSE', 'Sourcing Response'],
    ['PROC-EVALUATION', 'Sourcing Evaluation'],
    ['CBO-CONTRACT', 'Contract']
  ]);
  const nameFor = (id: string) => names.get(id) ?? externalNames.get(id) ?? id;
</script>

<svelte:head><title>Estimating & Tendering Semantics · NuBlox</title></svelte:head>

<div class="page">
  <header class="hero section-card">
    <div>
      <span class="eyebrow">Canonical pricing and bid model</span>
      <h1>Estimating, Measurement, Tendering, Proposals & Sales</h1>
      <p>Governed estimate identity, versioning, measurement, resource build-up, market testing, adjudication and customer-offer semantics without collapsing estimating into WBS, procurement, finance or Contract truth.</p>
    </div>
    <div class="hero-actions">
      ...
      <a href={`/${data.tenantSlug}/app/admin/business-objects/crm-business-development`}>CRM & business development</a>
      <a href={`/${data.tenantSlug}/app/admin/business-objects/commercial-procurement`}>Commercial & procurement</a>
      <strong>{estimatingTenderingModel.length} governed constructs</strong>
      <span>{estimatingTenderingRelationships.length} semantic relationships</span>
    </div>
  </header>

  <section class="section-card boundary-grid">
    {#each estimatingTenderingBoundaries as boundary}
      <article>
  ...
  <section class="section-card model">
    <div class="section-heading">
      <div><span class="eyebrow">Semantic contracts</span><h2>Governed estimating constructs</h2></div>
      <p>The exact internal pricing basis remains reconstructable while downstream offers, commitments, WBS and accounting retain their own identities.</p>
    </div>
    <div class="object-grid">
      {#each estimatingTenderingModel as item, index}
  ...
  <section class="section-card relationships">
    <div class="section-heading">
      <div><span class="eyebrow">Relationship graph</span><h2>From enquiry to accepted offer</h2></div>
      <p>Shared CRM and procurement objects are reused where the meaning is genuinely shared; estimating-specific identities remain independently governed.</p>
    </div>
    <div class="table-wrap">
      <table>
        <thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Cardinality</th><th>Governance</th></tr></thead>
        <tbody>
          {#each estimatingTenderingRelationships as relationship}
            <tr>
