...
async function assertLegalEntity(context: CommandContext, partyId: string | undefined, executor: DbExecutor) {
...
async function unitEvidence(context: CommandContext, row: OrganisationUnit, action: string, executor: DbExecutor, fromState?: string) {
...
export async function createOrganisationUnit(context: CommandContext, input: OrganisationUnitInput) {
  assertPermission(context, 'org.structure.manage');
  return dbTransaction(async (connection) => {
    const legalEntity = await assertLegalEntity(context, input.accountableLegalEntityPartyId, connection);
    const id = randomUUID();
    const timestamp = now();
    ...
    await executeMutation(
      ...
      [id, context.tenantId, required(input.unitCode, 'Organisation Unit code').toUpperCase(), required(input.name, 'Organisation Unit name'), required(input.unitType, 'Organisation Unit type').toUpperCase(), legalEntity, validFrom, validTo, timestamp, timestamp],
      connection
    );
    await unitEvidence(context, await getRow(context, id, connection), 'ORGANISATION_UNIT_CREATED', connection);
    return id;
  });
}

export async function updateOrganisationUnit(context: CommandContext, id: string, input: OrganisationUnitInput, expectedVersion: number) {
  assertPermission(context, 'org.structure.manage');
  return dbTransaction(async (connection) => {
    const current = await getRow(context, id, connection);
    if (current.version !== expectedVersion) throw new Error('This Organisation Unit changed after you opened it.');
    if (current.status === 'CLOSED') throw new Error('A closed Organisation Unit cannot be changed.');
    const legalEntity = await assertLegalEntity(context, input.accountableLegalEntityPartyId, connection);
    ...
    const result = await executeMutation(
      ...
      [required(input.unitCode, 'Organisation Unit code').toUpperCase(), required(input.name, 'Organisation Unit name'), required(input.unitType, 'Organisation Unit type').toUpperCase(), legalEntity, validFrom, validTo, now(), id, context.tenantId, expectedVersion],
      connection
    );
    if (result.affectedRows !== 1) throw new Error('Concurrent Organisation Unit change detected.');
    await unitEvidence(context, await getRow(context, id, connection), 'ORGANISATION_UNIT_CHANGED', connection, current.status);
  });
}

async function transition(context: CommandContext, id: string, expectedVersion: number, allowedFrom: string[], toState: string, action: string) {
  assertPermission(context, 'org.structure.manage');
  return dbTransaction(async (connection) => {
    const current = await getRow(context, id, connection);
    if (current.version !== expectedVersion) throw new Error('This Organisation Unit changed after you opened it.');
    if (!allowedFrom.includes(current.status)) throw new Error(`Organisation Unit cannot move from ${current.status} to ${toState}.`);
    const timestamp = now();
    const result = await executeMutation(
    ...
    if (result.affectedRows !== 1) throw new Error('Concurrent Organisation Unit lifecycle change detected.');
    await unitEvidence(context, await getRow(context, id, connection), action, connection, current.status);
  });
}

export const activateOrganisationUnit = (context: CommandContext, id: string, version: number) =>
  transition(context, id, version, ['PLANNED','INACTIVE'], 'ACTIVE', 'ORGANISATION_UNIT_ACTIVATED');
...
export const closeOrganisationUnit = (context: CommandContext, id: string, version: number) =>
  transition(context, id, version, ['PLANNED','ACTIVE','INACTIVE'], 'CLOSED', 'ORGANISATION_UNIT_CLOSED');
...
export async function assignOrganisationUnitParent(
  ...
  assertPermission(context, 'org.structure.manage');
  if (childUnitId === parentUnitId) throw new Error('An Organisation Unit cannot be its own parent.');
  return dbTransaction(async (connection) => {
    const child = await getRow(context, childUnitId, connection);
    const parent = await getRow(context, parentUnitId, connection);
    if (child.status === 'CLOSED' || parent.status === 'CLOSED') throw new Error('Closed Organisation Units cannot participate in the active hierarchy.');
    ...
    const current = await queryOne<RowDataPacket & { id: string }>(
      ...
      [context.tenantId, childUnitId], connection
    );
    if (current) {
      await executeMutation(
        "UPDATE organisation_unit_hierarchy SET status = 'ENDED', valid_to = ? WHERE id = ? AND tenant_id = ?",
        [timestamp, current.id, context.tenantId], connection
      );
    }
    ...
    await executeMutation(
      ...
      [relationId, context.tenantId, childUnitId, parentUnitId, timestamp, now()], connection
    );
    const result = await executeMutation(
      'UPDATE organisation_units SET version = version + 1, updated_at = ? WHERE id = ? AND tenant_id = ? AND version = ?',
      [now(), childUnitId, context.tenantId, child.version], connection
    );
    if (result.affectedRows !== 1) throw new Error('Concurrent Organisation Unit hierarchy change detected.');
    ...
    await emitBusinessEvent(context, {
      ...
      topic: 'nublox.organisation.structure',
      payload: { relationId, childUnitId, parentUnitId, replacedRelationId: current?.id ?? null, effectiveFrom: timestamp }
    }, connection);
    return relationId;
  });
}
