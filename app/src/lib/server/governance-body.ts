...
function validateMembers(input: GovernanceBodyInput) {
  ...
  for (const member of input.members) {
    ...
    if (unique.has(partyId))
      throw new Error('A Party may only appear once in a Governance Body membership set.');
    unique.set(partyId, {
      partyId,
      roleKey: code(member.roleKey, 'Governance Body membership role', 64)
    });
  }
  if (!unique.has(input.chairPartyId))
    throw new Error('Governance Body chair must be included in membership.');
  if (!unique.has(input.secretariatPartyId))
    throw new Error('Governance Body secretariat must be included in membership.');
  if (unique.size < input.quorumRequired)
    throw new Error('Governance Body quorum cannot exceed active membership.');
  ...
  return dbTransaction(async (connection) => {
    ...
    await executeMutation(
      ...
      [
        ...
        context.tenantId,
        bodyRef,
        required(input.name, 'Governance Body name'),
        bodyType,
        timestamp,
        timestamp
      ],
    ...
    await evidence(
      ...
      'PROPOSED',
      {
        bodyRef,
        bodyType,
        versionId: configured.versionId,
        memberCount: configured.members.length
      },
      connection
    );
    return id;
...
export async function reviseGovernanceBody(
  ...
  return dbTransaction(async (connection) => {
    ...
    if (current.aggregateVersion !== expectedAggregateVersion)
      throw new Error('This Governance Body changed after you opened it.');
    if (!['PROPOSED', 'SUSPENDED'].includes(current.status))
      throw new Error('Only proposed or suspended Governance Bodies can be reconfigured.');
    const nextVersionNo = current.currentVersionNo + 1;
    const configured = await insertVersion(
      context,
      current.id,
      nextVersionNo,
      current.status,
    ...
    const result = await executeMutation(
      ...
      [
        required(input.name, 'Governance Body name'),
        code(input.bodyType, 'Governance Body type', 64),
        nextVersionNo,
        timestamp,
        current.id,
        context.tenantId,
...
export async function addGovernanceBodyMember(
  ...
  return dbTransaction(async (connection) => {
    ...
    if (body.aggregateVersion !== expectedAggregateVersion)
      throw new Error('This Governance Body changed after you opened it.');
    if (body.status === 'DISSOLVED')
      throw new Error('Dissolved Governance Bodies cannot receive members.');
    ...
    const existing = await queryOne<RowDataPacket & { id: string }>(
      ...
      [context.tenantId, body.id, input.partyId],
      connection
    );
    ...
    await executeMutation(
      ...
      [
        ...
        body.id,
        input.partyId,
        code(input.roleKey, 'Membership role', 64),
    ...
    const result = await executeMutation(
      ...
      [timestamp, body.id, context.tenantId, expectedAggregateVersion],
      connection
    );
    if (result.affectedRows !== 1)
      throw new Error('Concurrent Governance Body membership change detected.');
    ...
    await evidence(
      ...
      body.status,
      {
        ...
        roleKey: code(input.roleKey, 'Membership role', 64)
      },
      connection
    );
  });
...
export async function endGovernanceBodyMembership(
  ...
  return dbTransaction(async (connection) => {
    ...
    const membership = await queryOne<RowDataPacket & GovernanceBodyMembership>(
      ...
      [membershipId, context.tenantId, body.id],
      connection
    );
    if (!membership || membership.status !== 'ACTIVE')
      throw new Error('Active Governance Body membership not found.');
    if (
      membership.partyId === body.chairPartyId ||
      membership.partyId === body.secretariatPartyId
    ) {
      throw new Error(
        'Chair or secretariat membership cannot end until Governance Body configuration is revised.'
      );
    }
    const active = await queryOne<RowDataPacket & { count: number }>(
      ...
      [context.tenantId, body.id],
      connection
    );
    if (Number(active?.count ?? 0) - 1 < body.quorumRequired)
      throw new Error('Membership cannot end because active membership would fall below quorum.');
    const timestamp = now();
    await executeMutation(
      ...
      [timestamp, timestamp, membership.id, context.tenantId],
      connection
    );
    const result = await executeMutation(
      'UPDATE governance_bodies SET aggregate_version = aggregate_version + 1, updated_at = ? WHERE id = ? AND tenant_id = ? AND aggregate_version = ?',
    ...
    await evidence(
      context,
      updated,
      'GOVERNANCE_BODY_MEMBERSHIP_ENDED',
      body.status,
      body.status,
      {
...
export async function transitionGovernanceBody(
  ...
  const transitions: Record<string, { from: string[]; to: string; event: string }> = {
    ...
    ACTIVATE: {
      from: ['CONSTITUTED', 'SUSPENDED'],
      to: 'ACTIVE',
      event: 'GOVERNANCE_BODY_ACTIVATED'
    },
    SUSPEND: { from: ['ACTIVE'], to: 'SUSPENDED', event: 'GOVERNANCE_BODY_SUSPENDED' },
    DISSOLVE: {
      from: ['CONSTITUTED', 'ACTIVE', 'SUSPENDED'],
      to: 'DISSOLVED',
      event: 'GOVERNANCE_BODY_DISSOLVED'
    }
  };
  ...
  return dbTransaction(async (connection) => {
    ...
    if (body.aggregateVersion !== expectedAggregateVersion)
      throw new Error('This Governance Body changed after you opened it.');
    if (!transition.from.includes(body.status))
      throw new Error(
        'Governance Body cannot transition from ' + body.status + ' using ' + action + '.'
      );
    if (action === 'CONSTITUTE' || action === 'ACTIVATE') {
      ...
      if (Number(active?.count ?? 0) < body.quorumRequired)
        throw new Error('Governance Body cannot activate without membership meeting quorum.');
    }
    const timestamp = now();
    const result = await executeMutation(
      ...
      [transition.to, timestamp, body.id, context.tenantId, expectedAggregateVersion],
      connection
    );
    if (result.affectedRows !== 1)
      throw new Error('Concurrent Governance Body transition detected.');
    await executeMutation(
      'UPDATE governance_body_versions SET lifecycle_status = ? WHERE body_id = ? AND version_no = ?',
      [transition.to, body.id, body.currentVersionNo],
      connection
    );
    if (action === 'DISSOLVE') {
      await executeMutation(
        ...
        [timestamp, timestamp, context.tenantId, body.id],
        connection
      );
    }
    const updated = await getBody(context, body.id, connection);
    await evidence(
      ...
      body.status,
      transition.to,
      { versionNo: body.currentVersionNo },
      connection
    );
  });
}
