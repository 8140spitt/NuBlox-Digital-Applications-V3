...
<div class="plan-page">
  <nav class="breadcrumb">
    <a href={`/${data.tenantSlug}/app/functions/f01`}>F01 Strategy & Enterprise Planning</a><span
      >›</span
    ><strong>F01.04 Business Planning</strong>
  </nav>

  <header class="hero section-card">
    <div>
      <span class="eyebrow">F01.04 · SGP-BUSINESS-PLAN</span>
      <h1>Business plans</h1>
      <p>
        Translate published strategy into owned, period-bound execution plans while preserving exact
        objective and assumption versions behind each approved baseline.
      </p>
    </div>
    <div class="principle">
      <strong>Plan ≠ budget or forecast truth</strong><span
        >Financial and resource values here are governed planning expectations.</span
      ><small
        >Approved plan baselines remain reconstructable when later revisions are created.</small
      >
    </div>
  </header>
  ...
  <div class="workspace-grid">
    <aside class="section-card register">
      <div>
        <span class="eyebrow">Plan register</span>
      ...
      <nav class="plan-list">
        {#each data.plans as item}
          <a class:active={data.selected?.id === item.id} href={href(item.id)}>
            <div>
              <strong>{item.planRef}</strong><span class={'status ' + item.status.toLowerCase()}
                >{item.status}</span
              >
            </div>
            <p>{item.name}</p>
            <small
              >{new Date(item.periodStart).getUTCFullYear()}–{new Date(
                item.periodEnd
              ).getUTCFullYear()} · v{item.currentVersionNo}</small
            >
          </a>
      ...
      {#if data.capabilities.canManage}
        <details class="command">
          ...
          <form method="POST" action="?/create">
            ...
            <label
              >Name<input name="name" required placeholder="FY27 Enterprise Business Plan" /></label
            >
            <label
            ...
            <label
              >Resource assumptions<textarea name="resourceAssumptions" rows="3" required
              ></textarea></label
            >
            <label
              >Financial expectations<textarea name="financialExpectations" rows="3" required
              ></textarea></label
            >
            <label
              >Measurable outcomes<textarea name="measurableOutcomes" rows="3" required
              ></textarea></label
            >
            <label
              >Delivery roadmap<textarea name="deliveryRoadmap" rows="3" required></textarea></label
            >
            <label
              >Strategic objectives<select name="objectives" multiple size="5"
                >{#each data.objectives as item}<option
                    value={`${item.id}:${item.currentVersionNo}`}
                    >{item.objectiveRef} · v{item.currentVersionNo}</option
                  >{/each}</select
              ></label
            >
            <label
              >Planning assumptions<select name="assumptions" multiple size="5"
                >{#each data.assumptions as item}<option
                    value={`${item.id}:${item.currentVersionNo}`}
                    >{item.assumptionRef} · v{item.currentVersionNo}</option
                  >{/each}</select
              ></label
            >
    ...
    <main class="main-column">
      {#if data.selected}
        <section class="section-card detail">
          <div class="detail-head">
            <div>
              ...
              <h2>{data.selected.planRef} · {data.selected.name}</h2>
            </div>
          ...
          <div class="facts">
            <span
              ><small>Plan version</small><strong>v{data.selected.currentVersionNo}</strong></span
            ><span><small>Aggregate</small><strong>v{data.selected.aggregateVersion}</strong></span
            ><span><small>Objectives</small><strong>{data.objectiveReferences.length}</strong></span
            ><span
              ><small>Assumptions</small><strong>{data.assumptionReferences.length}</strong></span
            >
          </div>
          <div class="content-grid">
            <article>
              <span class="eyebrow">Resource assumptions</span>
              <p>{data.selected.resourceAssumptions}</p>
            </article>
            <article>
              <span class="eyebrow">Financial expectations</span>
              <p>{data.selected.financialExpectations}</p>
            </article>
            <article>
              <span class="eyebrow">Measurable outcomes</span>
              <p>{data.selected.measurableOutcomes}</p>
            </article>
            <article>
              <span class="eyebrow">Delivery roadmap</span>
              <p>{data.selected.deliveryRoadmap}</p>
            </article>
          </div>
            ...
            {#if data.capabilities.canApprove && data.selected.status === 'APPROVED'}
              <form method="POST" action="?/activate">
                <input type="hidden" name="planId" value={data.selected.id} /><input
                  ...
                  name="aggregateVersion"
                  value={data.selected.aggregateVersion}
                /><button>Make current baseline</button>
              </form>
            {/if}
            {#if data.capabilities.canApprove && data.selected.status === 'CURRENT'}
              <form method="POST" action="?/close">
                <input type="hidden" name="planId" value={data.selected.id} /><input
                  ...
                  name="aggregateVersion"
                  value={data.selected.aggregateVersion}
                /><button class="quiet">Close plan</button>
              </form>
            {/if}
          </div>
          ...
          {#if data.capabilities.canManage && (data.selected.status === 'DRAFT' || data.selected.status === 'CURRENT')}
            <details class="command">
              ...
              <form method="POST" action="?/revise">
                ...
                <label
                  >Resource assumptions<textarea name="resourceAssumptions" rows="3" required
                    >{data.selected.resourceAssumptions}</textarea
                  ></label
                >
                <label
                  >Financial expectations<textarea name="financialExpectations" rows="3" required
                    >{data.selected.financialExpectations}</textarea
                  ></label
                >
                <label
                  >Measurable outcomes<textarea name="measurableOutcomes" rows="3" required
                    >{data.selected.measurableOutcomes}</textarea
                  ></label
                >
                <label
                  >Delivery roadmap<textarea name="deliveryRoadmap" rows="3" required
                    >{data.selected.deliveryRoadmap}</textarea
                  ></label
                >
                <label
                  >Strategic objectives<select name="objectives" multiple size="5"
                    >{#each data.objectives as item}<option
                        selected={isObjectiveSelected(item.id)}
                        value={`${item.id}:${item.currentVersionNo}`}
                        >{item.objectiveRef} · v{item.currentVersionNo}</option
                      >{/each}</select
                  ></label
                ...
                <label
                  >Planning assumptions<select name="assumptions" multiple size="5"
                    >{#each data.assumptions as item}<option
                        selected={isAssumptionSelected(item.id)}
                        value={`${item.id}:${item.currentVersionNo}`}
                        >{item.assumptionRef} · v{item.currentVersionNo}</option
                      >{/each}</select
                  ></label
        ...
        <section class="section-card history">
          <span class="eyebrow">Immutable baselines</span>
          ...
          <div class="version-list">
            {#each data.versions as version}<article>
                ...
                <p>{version.measurableOutcomes}</p>
      ...
      {:else}
        <section class="section-card empty-state">
          <h2>Create the first Business Plan</h2>
          <p>
            Publish Strategy and approve at least one Strategic Objective first, then convert that
            direction into an executable plan.
          </p>
        </section>
      {/if}
    </main>
