...
describe('F01.05 Operating Model runtime',()=>{
  it('preserves versioned target capabilities/accountabilities and governed approval',async()=>{
    ...
    const frameworkId=await strategy.createStrategyFramework(context,{
      title:'Operating Model Strategy',purpose:'Guide target operating model.',vision:'Integrated delivery.',mission:'Deliver through clear capabilities.',direction:'Standardise core services and accountabilities.',reviewCadence:'Annual'
    });
    await strategy.submitStrategyFramework(context,frameworkId);
    const strategyDecision=await decision.recordWorkDecision(context,{decisionType:'STRATEGY_FRAMEWORK_REVIEW',subjectType:'STRATEGY_FRAMEWORK',subjectId:frameworkId,subjectVersion:'1',outcome:'APPROVED',reason:'Approved for operating-model design.'});
    ...
    const modelId=await model.createOperatingModel(context,{
      ...
      sharedServiceRequirements:'Finance operations, people services, technology service desk and data platform.',
      organisationModelReference:'target-org-model-2027',
      changeInitiativesSummary:'Establish shared services, simplify governance and implement capability ownership.',
      capabilities:[
        {capabilityKey:'CAP-DIGITAL-DELIVERY',name:'Digital Delivery',description:'Integrated information and delivery control.',criticality:'CRITICAL',deliveryModel:'CENTRALISED'},
        {capabilityKey:'CAP-PROJECT-DELIVERY',name:'Project Delivery',description:'Client-facing project execution.',criticality:'CRITICAL',deliveryModel:'DECENTRALISED'}
      ],
      accountabilities:[
        {accountabilityKey:'ACC-DIGITAL',responsibility:'Own enterprise digital delivery capability.',accountableRoleKey:'CHIEF-DIGITAL-OFFICER',decisionRights:'Set platform standards and approve enterprise digital controls.'},
        {accountabilityKey:'ACC-DELIVERY',responsibility:'Own project-delivery performance.',accountableRoleKey:'COO',decisionRights:'Set delivery operating standards and intervene on material variance.'}
      ]
    });
    ...
    const approvalDecision=await decision.recordWorkDecision(context,{decisionType:'OPERATING_MODEL_APPROVAL',subjectType:'OPERATING_MODEL',subjectId:modelId,subjectVersion:String(current.currentVersionNo),outcome:'APPROVED',reason:'Target Operating Model approved.'});
    ...
    await model.reviseOperatingModel(context,modelId,current.aggregateVersion,{
      ...
      changeInitiativesSummary:'Complete shared-service mobilisation and regional accountability design.',
      capabilities:[
        {capabilityKey:'CAP-DIGITAL-DELIVERY',name:'Digital Delivery',description:'Integrated information and delivery control.',criticality:'CRITICAL',deliveryModel:'SHARED_SERVICE'}
      ],
      accountabilities:[
        {accountabilityKey:'ACC-DIGITAL',responsibility:'Own enterprise digital delivery capability.',accountableRoleKey:'CHIEF-DIGITAL-OFFICER',decisionRights:'Set platform standards and service levels.'}
      ]
    });
    current=(await model.listOperatingModels(context)).find(row=>row.id===modelId)!;
    expect(current.status).toBe('DRAFT');
    ...
    expect(versions.map(v=>[v.versionNo,v.lifecycleStatus])).toEqual([[2,'DRAFT'],[1,'ACTIVE']]);

    const events=await db.queryRows<any>("SELECT aggregate_version AS aggregateVersion FROM business_events WHERE tenant_id=? AND aggregate_id='AGG-02-STRATEGY' AND aggregate_type='OperatingModel' AND aggregate_object_id=? ORDER BY aggregate_version",[context.tenantId,modelId]);
