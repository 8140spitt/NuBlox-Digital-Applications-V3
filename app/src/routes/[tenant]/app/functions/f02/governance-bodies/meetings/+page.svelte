...
<div class="page">
  ...
  <header class="hero section-card">
    <div>
      ...
      <p>
        Schedule from effective Governance Body membership, pin exact issued meeting packs,
        establish quorum, record resolutions and coordinate accountable follow-up.
      </p>
    </div>
    <div class="principle">
    ...
    <nav>
      {#each data.bodies as body}
        <a class:active={data.body?.id === body.id} href={bodyHref(body.id)}>
          <strong>{body.bodyRef}</strong><span>{body.name}</span><small
            >{body.bodyType} · {body.status}</small
          >
        </a>
      {/each}
    </nav>
  ...
  {#if data.body}
    <div class="workspace">
      <aside class="section-card register">
        <div>
          <span class="eyebrow">{data.body.bodyType} schedule</span>
          <h2>{data.meetings.length} meetings</h2>
        </div>
        <nav class="meeting-list">
          {#each data.meetings as meeting}
            <a
              class:active={data.selected?.id === meeting.id}
              href={meetingHref(data.body.id, meeting.id)}
            >
              <div>
                <strong>{meeting.meetingRef}</strong><span
                  class={'status ' + meeting.status.toLowerCase()}>{meeting.status}</span
                >
              </div>
        ...
        {#if data.capabilities.canManage && data.body.status === 'ACTIVE'}
          <details class="command">
            ...
            <form method="POST" action="?/schedule">
              ...
              <label
                >Agenda<textarea name="agenda" rows="7" required></textarea>
                <small
      ...
      <main class="main">
        {#if data.selected}
          <section class="section-card detail">
            <div class="head">
              <div>
                <span class="eyebrow">{data.body.name}</span>
              ...
              <section>
                ...
                <div class="rows">
                  {#each data.agendaRows as row}
                    <article>
                      <div>
                        <strong>{row.itemNo}. {row.subject}</strong><span
                          >{row.requiredOutcome}</span
                        >
                      </div>
                      <p>{row.purpose}</p>
                      <small
          ...
          <section class="section-card panel">
            <div class="section-head">
              <div>
                <span class="eyebrow">Controlled meeting information</span>
                <h2>Exact issued revisions</h2>
              </div>
            </div>
            <div class="rows">
              {#each data.information as item}
                <article>
                  ...
                  <p>{item.title}</p>
                  <small
                    >Revision {item.revisionCode} · #{item.revisionNo} · {item.revisionId}</small
                  >
                </article>
              {:else}<p class="empty">
                  No issued Information Revisions linked to this meeting.
                </p>{/each}
            </div>

            {#if data.capabilities.canManage && data.capabilities.canReadInformation && data.selected.status !== 'COMPLETED'}
              <details class="command">
                <summary>Link issued paper / minutes</summary>
          ...
          {#if data.capabilities.canConduct && data.selected.status === 'CONVENED'}
            <div class="two">
              <section class="section-card panel">
                ...
                <h2>Immutable decision evidence</h2>
        ...
        {:else}
          <section class="section-card empty-state">
            ...
            <p>
              Meeting attendance will be snapshotted from the body’s effective membership at the
              scheduled time.
            </p>
          </section>
        {/if}
