<script lang="ts">
  import {
    ...
    transformationProcessImprovementRelationships,
    transformationProcessImprovementRules
  } from '$lib/data/transformation-process-improvement-model';

  let { data } = $props();
  const names = new Map(transformationProcessImprovementModel.map((x) => [x.modelId, x.canonicalName]));
  const external = new Map([
    ['DEL-PORTFOLIO','Portfolio'],['SGP-STRATEGIC-INITIATIVE','Strategic Initiative'],
    ['DEL-PROGRAMME','Programme'],['CBO-PROJECT','Project'],
    ['HCM-TRAINING-SESSION','Training Session'],['HCM-LEARNING-PLAN','Learning Plan'],
    ['WORK-FOLLOW-UP-ACTION','Decision Action'],['KRC-COMMUNICATION-ITEM','Communication Item'],
    ['AUTH-RESPONSIBILITY-ASSIGNMENT','Responsibility Assignment'],['SGP-KPI-DEFINITION','KPI Definition'],
    ['WORK-DECISION','Decision'],['CBO-INFORMATION-CONTAINER','Information Container'],
    ['COMP-ASSESSMENT','Compliance Assessment']
  ]);
  const nameFor=(id:string)=>names.get(id)??external.get(id)??id;
</script>

<svelte:head><title>Transformation & Process Improvement Semantics · NuBlox</title></svelte:head>

<div class="page">
  <header class="hero card">
    <div>
      <span class="eyebrow">Transformation and process governance</span>
      <h1>Organisation Change, Transformation & Continuous Improvement</h1>
      <p>Transformation initiatives, impact/readiness/adoption, organisation transition and enterprise process architecture built on shared portfolio, HCM, communication, decision and compliance foundations.</p>
    </div>
    <div class="actions">
      ...
      <a href={'/'+data.tenantSlug+'/app/admin/business-objects/strategy-governance-performance'}>Strategy & governance</a>
      <strong>{transformationProcessImprovementModel.length} governed constructs</strong>
      <span>{transformationProcessImprovementRelationships.length} relationships</span>
    </div>
  </header>

  <section class="card chain">
    <div><span>Transformation</span><strong>Portfolio → Initiative</strong></div>
    <div><span>Impact</span><strong>Assessment → Cohorts</strong></div>
    <div><span>Readiness</span><strong>Plan / Learning / Adoption</strong></div>
    <div><span>Transition</span><strong>Organisation cutover</strong></div>
    <div><span>Process</span><strong>Architecture → Process → Version → Improve</strong></div>
  </section>

  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Semantic contracts</span><h2>Governed transformation and process constructs</h2></div><p>Transformation coordinates shared enterprise truth; it does not clone portfolio, project, party, learning, communications or compliance objects.</p></div>
    <div class="grid">
      {#each transformationProcessImprovementModel as item,i}
        <details open={i<4}>
          <summary><span><code>{item.modelId}</code><strong>{item.canonicalName}</strong></span><small>{item.kind}</small></summary>
          <div class="body">
            <p>{item.definition}</p>
            <dl>
              <div><dt>Candidate</dt><dd>{item.candidateKeys.length ? item.candidateKeys.join(', ') : 'Derived canonical construct'}</dd></div>
              <div><dt>Identity</dt><dd>{item.identityRule}</dd></div>
              <div><dt>Lifecycle</dt><dd>{item.lifecycle.join(' · ')}</dd></div>
              <div><dt>Governance</dt><dd><ul>{#each item.governance as x}<li>{x}</li>{/each}</ul></dd></div>
            </dl>
          </div>
        </details>
      {/each}
  ...
  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Relationship graph</span><h2>Strategy-to-transition-to-process provenance</h2></div></div>
    <div class="table"><table><thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Governance</th></tr></thead><tbody>
      {#each transformationProcessImprovementRelationships as r}<tr><td><code>{r.id}</code></td><td>{nameFor(r.from)}</td><td>{r.predicate}</td><td>{nameFor(r.to)}</td><td>{r.governance}</td></tr>{/each}
    </tbody></table></div>
  </section>

  <section class="card content rules">
    <span class="eyebrow">Non-negotiable rules</span><h2>What NuBlox must preserve</h2>
    <ol>{#each transformationProcessImprovementRules as x}<li>{x}</li>{/each}</ol>
  </section>
</div>
