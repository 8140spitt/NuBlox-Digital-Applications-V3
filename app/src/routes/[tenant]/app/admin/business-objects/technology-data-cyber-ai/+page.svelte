<script lang="ts">
  import {
    ...
    technologyDataCyberAiRelationships,
    technologyDataCyberAiRules
  } from '$lib/data/technology-data-cyber-ai-model';

  let { data } = $props();
  const names = new Map(technologyDataCyberAiModel.map((x) => [x.modelId, x.canonicalName]));
  const external = new Map([
    ['CBO-ASSET','Asset'],['AUTH-USER-IDENTITY','User Identity'],['WORK-DECISION','Decision'],
    ['BCM-RECOVERY-REQUIREMENT','Recovery Requirement'],['BCM-DR-INVOCATION','Disaster Recovery Invocation'],
    ['RISK-ASSESSMENT','Risk Assessment'],['SGP-POLICY','Policy'],['ASSURANCE-REMEDIATION-ACTION','Remediation Action']
  ]);
  const nameFor=(id:string)=>names.get(id)??external.get(id)??id;
</script>

<svelte:head><title>IT, Data, Cyber & AI Semantics · NuBlox</title></svelte:head>

<div class="page">
  <header class="hero card">
    <div>
      <span class="eyebrow">Technology, data and digital governance</span>
      <h1>IT, Data, Cyber, Analytics & AI</h1>
      <p>Technology services and operations, configuration, data products, analytics, AI governance and cybersecurity around shared enterprise identities and evidence.</p>
    </div>
    <div class="actions">
      ...
      <a href={'/'+data.tenantSlug+'/app/admin/business-objects/continuity-crisis-security'}>Continuity & security</a>
      <strong>{technologyDataCyberAiModel.length} governed constructs</strong>
      <span>{technologyDataCyberAiRelationships.length} relationships</span>
    </div>
  </header>

  <section class="card chain">
    <div><span>Technology</span><strong>Service → Resource → Config</strong></div>
    <div><span>ITSM</span><strong>Request → Incident / Problem → Change / Release</strong></div>
    <div><span>Data</span><strong>Domain → Product → Dataset → Quality / Pipeline</strong></div>
    <div><span>AI</span><strong>Use Case → Model → Risk / Monitoring</strong></div>
    <div><span>Cyber</span><strong>Vulnerability / Alert → Incident → Finding</strong></div>
  </section>

  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Semantic contracts</span><h2>Governed technology, data and cyber constructs</h2></div><p>Digital operations reuse canonical asset, risk, policy, decision and authority foundations instead of maintaining duplicate masters.</p></div>
    <div class="grid">
      {#each technologyDataCyberAiModel as item,i}
        <details open={i<4}>
  ...
  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Relationship graph</span><h2>Service-to-data-to-security provenance</h2></div></div>
    <div class="table"><table><thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Governance</th></tr></thead><tbody>
      {#each technologyDataCyberAiRelationships as r}<tr><td><code>{r.id}</code></td><td>{nameFor(r.from)}</td><td>{r.predicate}</td><td>{nameFor(r.to)}</td><td>{r.governance}</td></tr>{/each}
    </tbody></table></div>
  </section>

  <section class="card content rules">
    <span class="eyebrow">Non-negotiable rules</span><h2>What NuBlox must preserve</h2>
    <ol>{#each technologyDataCyberAiRules as x}<li>{x}</li>{/each}</ol>
  </section>
</div>
