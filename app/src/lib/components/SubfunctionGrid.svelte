    ...
    {#each areas as area}
      <article class:operational={Boolean(area.path)} class="area-card">
        <header>
          <span>{area.id}</span>
          <h3>{area.name}</h3>
          <small>{area.sourceName}</small>
        </header>
        <p>{area.summary}</p>
        <ul>
          {#each area.items as item}<li>{item}</li>{/each}
        </ul>
        {#if area.path}
...
<style>
  ...
  .area-card {
    ...
    border-radius: 9px;
    background: white;
    box-shadow: 0 2px 7px rgba(31, 81, 111, 0.04);
  }
  .area-card.operational {
    border-color: #63afd8;
    box-shadow: 0 5px 14px rgba(31, 81, 111, 0.09);
  }
  .area-card header {
    min-height: 80px;
