...
describe('CRM and business development semantic model', () => {
  it('is internally valid and covers all BOF-03 candidates', () => {
    ...
    const decided = new Set(crmBusinessDevelopmentCanonicalization.map((entry) => entry.candidateKey));
  ...
  it('does not create duplicate customer/account organisation masters', () => {
    ...
    expect(rules).toContain('Prospect, customer and account terminology never creates duplicate Party or Organisation masters');
    for (const key of ['BOF-03-003', 'BOF-03-016', 'BOF-03-017']) {
      const decision = crmBusinessDevelopmentCanonicalization.find((entry) => entry.candidateKey === key);
  ...
  it('keeps Lead, Opportunity and Pursuit separate', () => {
    expect(crmBusinessDevelopmentModel.find((entry) => entry.modelId === 'CRM-LEAD')?.canonicalName).toBe('Lead');
    expect(crmBusinessDevelopmentModel.find((entry) => entry.modelId === 'CRM-OPPORTUNITY')?.canonicalName).toBe('Opportunity');
    expect(crmBusinessDevelopmentModel.find((entry) => entry.modelId === 'CRM-PURSUIT')?.canonicalName).toBe('Pursuit');
  ...
  it('treats decisions and interactions as evidence and forecasts as projections', () => {
    expect(crmBusinessDevelopmentModel.find((entry) => entry.modelId === 'CRM-BID-DECISION')?.kind).toBe('event-evidence');
    expect(crmBusinessDevelopmentModel.find((entry) => entry.modelId === 'CRM-INTERACTION')?.kind).toBe('event-evidence');
    expect(crmBusinessDevelopmentModel.find((entry) => entry.modelId === 'CRM-PIPELINE-SNAPSHOT')?.kind).toBe('projection');
    expect(crmBusinessDevelopmentModel.find((entry) => entry.modelId === 'CRM-FORECAST-SNAPSHOT')?.kind).toBe('projection');
  });

  it('normalizes Complaint into Customer Case', () => {
    const complaint = crmBusinessDevelopmentCanonicalization.find((entry) => entry.candidateKey === 'BOF-03-015');
