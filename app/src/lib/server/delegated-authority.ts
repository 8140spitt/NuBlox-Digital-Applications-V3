...
async function assertActiveParty(context: CommandContext, partyId: string, executor: DbExecutor) {
  const row = await queryOne<RowDataPacket & { id: string }>(
    ...
    [partyId, context.tenantId], executor
  );
...
async function evidence(context: CommandContext, row: DelegatedAuthority, action: string, executor: DbExecutor, fromState?: string) {
...
export async function listDelegatedAuthorities(context: CommandContext) {
  ...
  return queryRows<RowDataPacket & DelegatedAuthority>(
    selectAuthority + ' WHERE da.tenant_id = ? ORDER BY dp.display_name, da.authority_type, da.valid_from DESC',
    [context.tenantId]
  );
}

export async function createDelegatedAuthority(context: CommandContext, input: DelegatedAuthorityInput) {
  ...
  if (input.valueLimit != null && (!Number.isFinite(input.valueLimit) || input.valueLimit < 0)) throw new Error('Value limit must be a non-negative number.');
  const currencyCode = input.currencyCode?.trim().toUpperCase() || null;
  if (currencyCode && !/^[A-Z]{3}$/.test(currencyCode)) throw new Error('Currency code must be a three-letter ISO-style code.');
  if (input.valueLimit != null && !currencyCode) throw new Error('A monetary value limit requires a currency code.');
  ...
  return dbTransaction(async (connection) => {
    ...
    await executeMutation(
      ...
      [id, context.tenantId, grantorPartyId, input.delegatePartyId, required(input.authorityType, 'Authority type').toUpperCase(), required(input.basis, 'Authority basis'), required(input.scopeType, 'Scope type').toUpperCase(), required(input.scopeId, 'Scope ID'), currencyCode, input.valueLimit ?? null, input.allowSubdelegation ? 1 : 0, validFrom, validTo, timestamp, timestamp],
      connection
    );
    await evidence(context, await getRow(context, id, connection), 'DELEGATED_AUTHORITY_CREATED', connection);
    return id;
  });
}
...
async function transition(
  ...
  return dbTransaction(async (connection) => {
    ...
    if (current.version !== expectedVersion) throw new Error('This Delegated Authority changed after you opened it.');
    if (!allowedFrom.includes(current.status)) throw new Error(`Delegated Authority cannot move from ${current.status} to ${toState}.`);
    ...
    const timestamp = now();
    const revocationReason = toState === 'REVOKED' ? required(reason ?? '', 'Revocation reason') : current.revocationReason;
    const result = await executeMutation(
      ...
      [toState, toState, timestamp, toState, timestamp, revocationReason, timestamp, id, context.tenantId, expectedVersion],
      connection
    );
    if (result.affectedRows !== 1) throw new Error('Concurrent Delegated Authority change detected.');
    await evidence(context, await getRow(context, id, connection), action, connection, current.status);
  });
}

export const approveDelegatedAuthority = (context: CommandContext, id: string, version: number) =>
  transition(context, id, version, 'authority.delegation.approve', ['DRAFT'], 'APPROVED', 'DELEGATED_AUTHORITY_APPROVED');
export const activateDelegatedAuthority = (context: CommandContext, id: string, version: number) =>
  transition(context, id, version, 'authority.delegation.manage', ['APPROVED','SUSPENDED'], 'ACTIVE', 'DELEGATED_AUTHORITY_ACTIVATED');
export const suspendDelegatedAuthority = (context: CommandContext, id: string, version: number) =>
  transition(context, id, version, 'authority.delegation.manage', ['ACTIVE'], 'SUSPENDED', 'DELEGATED_AUTHORITY_SUSPENDED');
export const revokeDelegatedAuthority = (context: CommandContext, id: string, version: number, reason: string) =>
  transition(context, id, version, 'authority.delegation.manage', ['DRAFT','APPROVED','ACTIVE','SUSPENDED'], 'REVOKED', 'DELEGATED_AUTHORITY_REVOKED', reason);
...
export async function findEffectiveDelegatedAuthority(
  ...
  const rows = await queryRows<RowDataPacket & DelegatedAuthority>(
    selectAuthority + " WHERE da.tenant_id = ? AND da.delegate_party_id = ? AND da.authority_type = ? AND da.scope_type = ? AND da.scope_id = ? AND da.status = 'ACTIVE' AND da.valid_from <= ? AND (da.valid_to IS NULL OR da.valid_to > ?) ORDER BY da.value_limit DESC",
    [context.tenantId, input.delegatePartyId ?? context.actorPartyId, required(input.authorityType, 'Authority type').toUpperCase(), required(input.scopeType, 'Scope type').toUpperCase(), required(input.scopeId, 'Scope ID'), timestamp, timestamp]
  );

  if (input.value == null) return rows[0] ?? null;
  if (!Number.isFinite(input.value) || input.value < 0) throw new Error('Authority evaluation value must be a non-negative number.');
