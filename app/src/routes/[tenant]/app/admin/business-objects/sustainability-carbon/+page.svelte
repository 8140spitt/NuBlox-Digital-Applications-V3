<script lang="ts">
  import {
    ...
    sustainabilityCarbonRelationships,
    sustainabilityCarbonRules
  } from '$lib/data/sustainability-carbon-model';

  let { data } = $props();
  const names = new Map(sustainabilityCarbonModel.map((x) => [x.modelId, x.canonicalName]));
  const external = new Map([
    ['CBO-ITEM', 'Item'],
    ['OPS-UTILITY-CONSUMPTION', 'Utility Consumption'],
    ['QHSE-WASTE-CONSIGNMENT', 'Waste Consignment'],
    ['PROC-EVALUATION', 'Sourcing Evaluation'],
    ['CBO-PROJECT', 'Project'],
    ['RISK-ENTERPRISE-RISK', 'Enterprise Risk'],
    ['CBO-ASSET', 'Asset']
  ]);
  const nameFor = (id: string) => names.get(id) ?? external.get(id) ?? id;
</script>

<svelte:head><title>Sustainability & Carbon Semantics · NuBlox</title></svelte:head>

<div class="page">
  <header class="hero card">
    <div>
      <span class="eyebrow">Whole-life sustainability model</span>
      <h1>Sustainability, Carbon, Energy, Circularity & Social Value</h1>
      <p>Versioned methodologies, targets, evidence and assessments around canonical Items, Assets, Projects, utilities, waste, suppliers and finance—without creating a second sustainability truth store.</p>
    </div>
    <div class="actions">
      ...
      <strong>{sustainabilityCarbonModel.length} governed constructs</strong>
      <span>{sustainabilityCarbonRelationships.length} relationships</span>
    </div>
  </header>

  <section class="card chain">
    <div><span>Carbon control</span><strong>Method → Baseline → Budget → Target</strong></div>
    <div><span>Assessment</span><strong>Activity / Item / Utility → Carbon</strong></div>
    <div><span>Circularity</span><strong>Waste → Recovery / Reuse → Assessment</strong></div>
    <div><span>Social value</span><strong>Commitment → Evidence → Outcome</strong></div>
    <div><span>Risk & options</span><strong>Enterprise Risk / Cost-Carbon decisions</strong></div>
  </section>

  <section class="card content">
    <div class="heading">
      <div><span class="eyebrow">Semantic contracts</span><h2>Governed sustainability constructs</h2></div>
      <p>Calculated positions remain reproducible from exact source evidence, factor sets and methodology versions.</p>
    </div>
    <div class="grid">
      {#each sustainabilityCarbonModel as item, i}
        <details open={i < 4}>
          <summary>
  ...
  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Relationship graph</span><h2>Source-to-assessment provenance</h2></div></div>
    <div class="table">
      <table>
        <thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Governance</th></tr></thead>
        <tbody>
          {#each sustainabilityCarbonRelationships as r}
            <tr><td><code>{r.id}</code></td><td>{nameFor(r.from)}</td><td>{r.predicate}</td><td>{nameFor(r.to)}</td><td>{r.governance}</td></tr>
          {/each}
        </tbody>
      </table>
    </div>
  ...
  <section class="card content rules">
    ...
    <ol>{#each sustainabilityCarbonRules as x}<li>{x}</li>{/each}</ol>
  </section>
</div>
