<script lang="ts">
  ...
  const draftVersion = $derived(
    data.versions.find((version) => version.status === 'DRAFT') ?? null
  );
...
<div class="authority-page">
  <header class="hero section-card">
    ...
    <div class="principle">
      ...
      <small
        >Material rule changes create new immutable versions rather than rewriting history.</small
      >
    </div>
  </header>
            ...
            <form method="POST" action="?/createApproval">
              <label
                >Rule key<input
                  name="ruleKey"
                  required
                  placeholder="COMMERCIAL.APPROVAL.TIER1"
                /></label
              >
              <label
                >Protected action<input
                  name="actionKey"
                  required
                  placeholder="COMMERCIAL_APPROVAL"
                /></label
              >
              <label
                >Object type<input
            ...
            <form method="POST" action="?/createDelegated">
              <label
                >Rule key<input
                  name="ruleKey"
                  required
                  placeholder="COMMERCIAL.DELEGATION"
                /></label
              >
              <label
              ...
              >
              <label
                >Allowed scope ID<input
                  name="allowedScopeId"
                  placeholder="Optional exact scope"
                /></label
              >
              <div class="form-grid">
                <label>Currency<input name="currencyCode" maxlength="3" placeholder="GBP" /></label>
    ...
    <main class="main-column">
      {#if selectedRule}
        <section class="rule-header section-card">
          <div>
            <span class="eyebrow"
              >{kind === 'approval' ? 'Approval Authority Rule' : 'Delegated Authority Rule'}</span
            >
            ...
            {#each data.versions as version}
              <article>
                <div>
                  ...
                  <span class:published={version.status === 'PUBLISHED'} class="status"
                    >{version.status}</span
                  >
                </div>
                {#if kind === 'approval'}
                  <p>
                    ...
                    {#if version.maximumValue}
                      · max {Number(version.maximumValue).toLocaleString('en-GB')}{/if}
                  </p>
                {:else}
                  <p>
                    {version.allowedScopeType}
                    {#if version.maximumValue}
                      · max {Number(version.maximumValue).toLocaleString('en-GB')}
                      {version.currencyCode}{/if}
                    {#if version.maximumDurationDays}
                      · {version.maximumDurationDays} days{/if}
                  </p>
                {/if}
                {#if version.status === 'DRAFT' && data.capabilities.canPublish}
                  <form
                    method="POST"
                    action={kind === 'approval' ? '?/publishApproval' : '?/publishDelegated'}
                  >
                    <input type="hidden" name="ruleId" value={selectedRule.id} />
      ...
      {:else}
        <section class="empty-state section-card">
          ...
          <p>
            Published policy constrains runtime decisions and grants; it does not create authority
            itself.
          </p>
        </section>
      {/if}
