...
describe('people, HCM, competence, time, payroll and expenses semantic model', () => {
  ...
  it('reuses canonical Person for workers and candidates', () => {
    const candidate = peopleHcmCanonicalization.find((entry) => entry.candidateKey === 'BOF-18-036');
  ...
  it('separates Position, Job Profile and business Role Assignment', () => {
    expect(peopleHcmModel.find((entry) => entry.modelId === 'HCM-POSITION')?.canonicalName).toBe('Position');
    expect(peopleHcmModel.find((entry) => entry.modelId === 'HCM-JOB-PROFILE')?.canonicalName).toBe('Job Profile');
  ...
  it('keeps competence definition, learning evidence and person attainment separate', () => {
    expect(peopleHcmModel.find((entry) => entry.modelId === 'HCM-COMPETENCE-DEFINITION')?.kind).toBe('controlled-definition');
    expect(peopleHcmModel.find((entry) => entry.modelId === 'HCM-LEARNING-RECORD')?.kind).toBe('event-evidence');
    expect(peopleHcmModel.find((entry) => entry.modelId === 'HCM-PERSON-COMPETENCE')?.kind).toBe('relationship');
    expect(peopleHcmRules.join(' ')).toContain('Learning completion never automatically proves competence');
  });

  it('normalizes credential types and CPD/training records', () => {
    for (const key of ['BOF-18-011', 'BOF-18-012', 'BOF-18-013']) {
      expect(peopleHcmCanonicalization.find((entry) => entry.candidateKey === key)?.targetCandidateKey).toBe('BOF-18-010');
    }
    expect(peopleHcmCanonicalization.find((entry) => entry.candidateKey === 'BOF-18-017')?.targetCandidateKey).toBe('BOF-18-016');
  });

  it('keeps attendance, time and payroll as separate truth layers', () => {
    expect(peopleHcmModel.find((entry) => entry.modelId === 'HCM-ATTENDANCE')?.kind).toBe('event-evidence');
    expect(peopleHcmModel.find((entry) => entry.modelId === 'HCM-TIME-ENTRY')?.kind).toBe('transaction');
    expect(peopleHcmModel.find((entry) => entry.modelId === 'HCM-TIMESHEET')?.kind).toBe('transaction');
    expect(peopleHcmModel.find((entry) => entry.modelId === 'HCM-PAYROLL-RESULT')?.kind).toBe('event-evidence');
  });

  it('treats availability as projection and payslip as controlled information', () => {
    expect(peopleHcmCanonicalization.find((entry) => entry.candidateKey === 'BOF-18-019')?.decision).toBe('PROJECTION');
