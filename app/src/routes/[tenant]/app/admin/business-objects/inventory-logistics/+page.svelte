<script lang="ts">
  import {
    inventoryLogisticsBoundaries,
    inventoryLogisticsModel,
    inventoryLogisticsRelationships,
    inventoryLogisticsRules
  } from '$lib/data/inventory-logistics-model';

  let { data } = $props();
  const names = new Map(inventoryLogisticsModel.map((item) => [item.modelId, item.canonicalName]));
...
<svelte:head><title>Inventory & Logistics Semantics · NuBlox</title></svelte:head>

<div class="page">
  <header class="hero section-card">
    <div>
      <span class="eyebrow">Canonical inventory model</span>
      <h1>Inventory & Logistics Semantics</h1>
      <p>Governed storage, reservation, movement, stock-control and logistics execution semantics. Item identity, quantity, traceability, storage and delivery are deliberately separate layers.</p>
    </div>
    <div class="hero-actions">
      <a href={`/${data.tenantSlug}/app/admin/business-objects`}>← Canonicalization workbench</a>
      <a href={`/${data.tenantSlug}/app/admin/business-objects/item-manufacturing`}>Item & manufacturing</a>
      <a href={`/${data.tenantSlug}/app/admin/business-objects/commercial-procurement`}>Commercial & procurement</a>
      <strong>{inventoryLogisticsModel.length} governed constructs</strong>
      <span>{inventoryLogisticsRelationships.length} semantic relationships</span>
    </div>
  </header>

  <section class="section-card boundary-grid">
    {#each inventoryLogisticsBoundaries as boundary}
      <article>
  ...
  <section class="section-card model">
    <div class="section-heading">
      <div><span class="eyebrow">Semantic contracts</span><h2>Governed inventory and logistics constructs</h2></div>
      <p>Each construct defines identity, scope, lifecycle and the boundary that prevents duplicate stock or logistics truth.</p>
    </div>
    <div class="object-grid">
      {#each inventoryLogisticsModel as item, index}
  ...
  <section class="section-card relationships">
    <div class="section-heading">
      <div><span class="eyebrow">Relationship graph</span><h2>How inventory truth and logistics execution connect</h2></div>
      <p>Relationships preserve source/destination, demand, traceability and custody without collapsing the semantic layers.</p>
    </div>
    <div class="table-wrap">
      <table>
        <thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Cardinality</th><th>Governance</th></tr></thead>
        <tbody>
          {#each inventoryLogisticsRelationships as relationship}
            <tr>
