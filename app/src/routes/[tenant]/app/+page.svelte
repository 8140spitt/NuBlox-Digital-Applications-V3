<script lang="ts">
  ...
  const mappedFunctions = $derived(
    data.functions.filter((item) => item.implementationState === 'planned')
  );
  const operationalFunctions = $derived(
    data.functions.filter((item) => item.implementationState === 'active')
  );
</script>
...
<div class="home-page">
  ...
  <section class="focus-grid">
          ...
          <a href={'/' + data.tenantSlug + '/app/work'}>
            ...
            <span
              ><strong>{item.title}</strong><small
                >{item.status.replaceAll('_', ' ')} · {item.workType}</small
              ></span
            >
          </a>
        {:else}
    ...
    <article class="section-card focus-card">
      <div class="card-heading">
        <div>
          <span class="eyebrow">Enterprise foundation</span>
          <h2>Authoritative shared services</h2>
        </div>
      </div>
      <div class="foundation-links">
        <a href={'/' + data.tenantSlug + '/app/admin/master-data/parties'}
          ><strong>Party master</strong><span
            >People, organisations, legal entities & relationships</span
          ></a
        >
        <a href={'/' + data.tenantSlug + '/app/admin/master-data/organisation-structure'}
          ><strong>Organisation structure</strong><span
            >Units, hierarchy & legal accountability</span
          ></a
        >
        <a href={'/' + data.tenantSlug + '/app/admin/security'}
          ><strong>Security & authority</strong><span
            >Identity, membership, RBAC & delegated authority</span
          ></a
        >
        <a href={'/' + data.tenantSlug + '/app/admin/business-objects'}
          ><strong>Architecture authority</strong><span
            >Canonical objects, aggregate coverage & decisions</span
          ></a
        >
      </div>
    </article>
      ...
      {#each data.functions as fn}
        <a href={'/' + data.tenantSlug + '/app/functions/' + fn.id.toLowerCase()}>
          ...
          <span class="fn-copy">
            ...
            <small
              >{fn.subfunctionCount} L2 · {fn.activityCount} activities · {fn.aggregateCount} aggregates</small
            >
          </span>
...
<style>
  ...
  .foundation-links strong {
    color: #36566b;
    font-size: 10px;
  }
  .foundation-links span {
    color: #7a8b97;
    font-size: 8.5px;
    line-height: 1.35;
  }
