...
describe('knowledge, records, communications and stakeholder semantic model', () => {
  it('is internally valid and covers all 21 BOF-25 candidates', () => {
    ...
    const decided = new Set(
      knowledgeRecordsCommunicationsCanonicalization.map((x) => x.candidateKey)
    );
    expect(decided.size).toBe(21);
    for (let i = 1; i <= 21; i += 1)
      expect(decided.has(`BOF-25-${String(i).padStart(3, '0')}`)).toBe(true);
  });
  ...
  it('models declared record as a relationship rather than duplicate content', () => {
    const d = knowledgeRecordsCommunicationsCanonicalization.find(
      (x) => x.candidateKey === 'BOF-25-005'
    );
    expect(d?.decision).toBe('RELATIONSHIP');
    expect(d?.proposedCanonicalName).toBe('Record Declaration');
    expect(knowledgeRecordsCommunicationsRules.join(' ')).toContain(
      'never copies the underlying object'
    );
  });

  it('separates disposition request from retention disposition decision', () => {
    expect(
      knowledgeRecordsCommunicationsCanonicalization.find((x) => x.candidateKey === 'BOF-25-009')
        ?.decision
  ...
  it('normalizes reputation and public-affairs issues to one case pattern', () => {
    expect(
      knowledgeRecordsCommunicationsCanonicalization.find((x) => x.candidateKey === 'BOF-25-017')
        ?.proposedCanonicalName
    ).toBe('External Affairs Issue');
    const publicAffairs = knowledgeRecordsCommunicationsCanonicalization.find(
      (x) => x.candidateKey === 'BOF-25-018'
    );
  ...
  it('treats investor engagement as stakeholder engagement evidence', () => {
    const d = knowledgeRecordsCommunicationsCanonicalization.find(
      (x) => x.candidateKey === 'BOF-25-019'
    );
    expect(d?.decision).toBe('RENAME');
    expect(d?.proposedCanonicalName).toBe('Stakeholder Engagement');
    expect(
      knowledgeRecordsCommunicationsModel.find((x) => x.modelId === 'KRC-STAKEHOLDER-ENGAGEMENT')
        ?.kind
    ).toBe('event-evidence');
  });
});
