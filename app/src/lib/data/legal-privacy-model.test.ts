...
import {
  legalPrivacyModel,
  legalPrivacyRules,
  validateLegalPrivacyModel
} from './legal-privacy-model';
...
describe('legal, privacy and records-obligation semantic model', () => {
  it('is internally valid and covers all 21 BOF-22 candidates', () => {
    ...
    for (let i = 1; i <= 21; i += 1)
      expect(decided.has(`BOF-22-${String(i).padStart(3, '0')}`)).toBe(true);
  });
  ...
  it('keeps matter, dispute and proceeding identities distinct', () => {
    ...
    expect(legalPrivacyModel.find((x) => x.modelId === 'LEGAL-PROCEEDING')?.canonicalName).toBe(
      'Legal Proceeding'
    );
  });
  ...
  it('keeps processing definition and DPIA evidence separate', () => {
    expect(legalPrivacyModel.find((x) => x.modelId === 'PRIV-PROCESSING-ACTIVITY')?.kind).toBe(
      'processing-definition'
    );
    expect(
      legalPrivacyCanonicalization.find((x) => x.candidateKey === 'BOF-22-014')?.decision
    ).toBe('EVENT_EVIDENCE');
  });
  ...
  it('normalizes privacy breach to privacy incident type', () => {
    ...
    expect(legalPrivacyRules.join(' ')).toContain(
      'Privacy Breach is a Privacy Incident classification'
    );
  });

  it('models international transfer as a relationship and reuses assurance review', () => {
    expect(
      legalPrivacyCanonicalization.find((x) => x.candidateKey === 'BOF-22-020')?.decision
    ).toBe('RELATIONSHIP');
