...
<div class="page">
  ...
  <header class="hero section-card">
    <div>
      ...
      <p>
        Constitute Boards, Committees and Steering Bodies with stable identity, immutable terms,
        effective membership and explicit quorum.
      </p>
    </div>
    <div class="principle">
      ...
      <small
        >Decision rights remain governed by Authority Framework, role and Delegated Authority.</small
      >
    </div>
  </header>
  ...
  <div class="workspace">
    <aside class="section-card register">
      <div>
        <span class="eyebrow">Body register</span>
        <h2>{data.bodies.length} bodies</h2>
      </div>
      <nav class="body-list">
        {#each data.bodies as body}
          <a class:active={data.selected?.id === body.id} href={href(body.id)}>
            <div>
              <strong>{body.bodyRef}</strong><span class={'status ' + body.status.toLowerCase()}
                >{body.status}</span
              >
            </div>
            <p>{body.name}</p>
            <small>{body.bodyType} · v{body.currentVersionNo}</small>
          </a>
      ...
      {#if data.capabilities.canManage}
        <details class="command">
          ...
          <form method="POST" action="?/create">
            ...
            <label
              >Type<select name="bodyType"
                ><option>BOARD</option><option>COMMITTEE</option><option>STEERING_BODY</option
                ></select
              ></label
            >
            <label>Mandate<textarea name="mandate" rows="3" required></textarea></label>
            <label
              >Terms of reference<textarea name="termsOfReference" rows="4" required
              ></textarea></label
            >
            <div class="grid">
              ...
              >
              <label
                >Chair Party ID<input
                  name="chairPartyId"
                  required
                  value={data.actorPartyId}
                /></label
              >
              <label>Secretariat Party ID<input name="secretariatPartyId" required /></label>
            </div>
            <label
              >Membership rules<textarea name="membershipRules" rows="3" required></textarea></label
            >
            <label
              >Members<textarea name="members" rows="6" required></textarea><small
                >Party ID | CHAIR / SECRETARY / MEMBER / OBSERVER</small
              ></label
            >
    ...
    <main class="main">
      {#if data.selected}
        <section class="section-card detail">
          <div class="head">
            <div>
              <span class="eyebrow">{data.selected.bodyType}</span>
              <h2>{data.selected.bodyRef} · {data.selected.name}</h2>
            </div>
          ...
          <div class="meeting-link">
            <a
              href={`/${data.tenantSlug}/app/functions/f02/governance-bodies/meetings?body=${encodeURIComponent(data.selected.id)}`}
              >Open Board / Committee meetings →</a
            >
          </div>
          <div class="facts">
            <span><small>Aggregate</small><strong>v{data.selected.aggregateVersion}</strong></span>
            <span
              ><small>Configuration</small><strong>v{data.selected.currentVersionNo}</strong></span
            >
            <span><small>Quorum</small><strong>{data.selected.quorumRequired}</strong></span>
            <span
              ><small>Active members</small><strong
                >{data.memberships.filter((row) => row.status === 'ACTIVE').length}</strong
              ></span
            >
          </div>
          <div class="cards">
            <article>
              ...
              <p>{data.selected.mandate}</p>
            </article>
            <article>
              <span class="eyebrow">Terms of reference</span>
              <p>{data.selected.termsOfReference}</p>
            </article>
            <article>
              <span class="eyebrow">Membership rules</span>
              <p>{data.selected.membershipRules}</p>
            </article>
            <article>
              <span class="eyebrow">Governance scope</span>
              <p>{data.selected.scopeType} · {data.selected.scopeId}</p>
            </article>
          </div>
          ...
          {#if data.capabilities.canApprove}
            <form method="POST" action="?/transition" class="actions">
              ...
              {#if data.selected.status === 'PROPOSED'}<button name="action" value="CONSTITUTE"
                  >Constitute body</button
                >{/if}
              {#if data.selected.status === 'CONSTITUTED' || data.selected.status === 'SUSPENDED'}<button
                  name="action"
                  value="ACTIVATE">Activate</button
                >{/if}
              {#if data.selected.status === 'ACTIVE'}<button
                  class="quiet"
                  name="action"
                  value="SUSPEND">Suspend</button
                >{/if}
              {#if ['CONSTITUTED', 'ACTIVE', 'SUSPENDED'].includes(data.selected.status)}<button
                  class="danger"
                  name="action"
                  value="DISSOLVE">Dissolve</button
                >{/if}
            </form>
          {/if}
        </section>
        ...
        <div class="two">
          <section class="section-card panel">
            <span class="eyebrow">Effective membership</span>
            <h2>{data.memberships.length} membership records</h2>
            <div class="rows">
              {#each data.memberships as member}
                <article>
                  <div>
                    <strong>{member.roleKey}</strong><span
                      class={'status ' + member.status.toLowerCase()}>{member.status}</span
                    >
                  </div>
          ...
          <section class="section-card panel">
            <span class="eyebrow">Immutable configuration</span>
            ...
            <div class="rows">
              {#each data.versions as version}
                <article>
                  ...
                  <p>{version.mandate}</p>
                  <small>Quorum {version.quorumRequired}</small>
                </article>
              {/each}
            </div>
          </section>
        ...
        {#if data.capabilities.canManage && (data.selected.status === 'PROPOSED' || data.selected.status === 'SUSPENDED')}
          <section class="section-card panel">
            <details class="command" open>
              <summary>Revise governance configuration</summary>
              <form method="POST" action="?/revise">
                ...
                <label
                  >Type<select name="bodyType"
                    ><option selected={data.selected.bodyType === 'BOARD'}>BOARD</option><option
                      selected={data.selected.bodyType === 'COMMITTEE'}>COMMITTEE</option
                    ><option selected={data.selected.bodyType === 'STEERING_BODY'}
                      >STEERING_BODY</option
                    ></select
                  ></label
                >
                <label
                  >Mandate<textarea name="mandate" rows="3" required
                    >{data.selected.mandate}</textarea
                  ></label
                >
                <label
                  >Terms of reference<textarea name="termsOfReference" rows="4" required
                    >{data.selected.termsOfReference}</textarea
                  ></label
                >
                <div class="grid">
                  <label
                  ...
                  >
                  <label
                    >Scope ID<input name="scopeId" required value={data.selected.scopeId} /></label
                  >
                  <label
                    >Quorum<input
                      name="quorumRequired"
                      ...
                      min="1"
                      required
                      value={data.selected.quorumRequired}
                    /></label
                  >
                  <label
                    >Chair Party ID<input
                      name="chairPartyId"
                      required
                      value={data.selected.chairPartyId}
                    /></label
                  >
                  <label
                    >Secretariat Party ID<input
                      name="secretariatPartyId"
                      required
                      value={data.selected.secretariatPartyId}
                    /></label
                  >
                </div>
                <label
                  >Membership rules<textarea name="membershipRules" rows="3" required
                    >{data.selected.membershipRules}</textarea
                  ></label
                >
                <label
                  >Members<textarea name="members" rows="6" required value={memberRows()}
                  ></textarea></label
                >
      ...
      {:else}
        <section class="section-card empty-state">
          <h2>Constitute the first Governance Body</h2>
          <p>
            Create the Board or Committee identity, its mandate, quorum and effective membership
            here.
          </p>
        </section>
      {/if}
