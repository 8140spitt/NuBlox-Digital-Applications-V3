<script lang="ts">
  import {
    legalPrivacyModel,
    legalPrivacyRelationships,
    legalPrivacyRules
  } from '$lib/data/legal-privacy-model';
  let { data } = $props();
  ...
  const external = new Map([
    ...
    ['QHSE-ASSURANCE-REVIEW', 'Assurance Review'],
    ['CBO-CONTRACT', 'Contract'],
    ['CBO-PARTY', 'Party'],
    ['COMP-REQUIREMENT', 'Compliance Requirement']
  ]);
...
<div class="page">
  <header class="hero card">
    <div>
      <span class="eyebrow">Legal and privacy governance</span>
      <h1>Legal, Privacy & Records Obligations</h1>
      <p>
        Legal matters, filings, IP, preservation, privacy processing, rights requests and incident
        evidence around the same canonical people, parties, contracts and information.
      </p>
    </div>
    <div class="actions">
      ...
      ><strong>{legalPrivacyModel.length} governed constructs</strong><span
        >{legalPrivacyRelationships.length} relationships</span
      >
    </div>
  </header>
  ...
  <section class="card content">
    <div class="heading">
      <div>
        ...
        <h2>Governed legal and privacy constructs</h2>
      </div>
      <p>
        Legal and privacy context augments enterprise truth without becoming a parallel master-data
        estate.
      </p>
    </div>
    <div class="grid">
      {#each legalPrivacyModel as item, i}<details open={i < 4}>
          <summary
  ...
  <section class="card content">
    <div class="heading">
      <div>
        ...
        <h2>Legal/privacy provenance</h2>
      </div>
    </div>
    <div class="table">
      <table>
        <thead
          ...
          ></thead
        ><tbody
          >{#each legalPrivacyRelationships as r}<tr
  ...
  <section class="card content rules">
    ...
    <ol>
      {#each legalPrivacyRules as x}<li>{x}</li>{/each}
    </ol>
  </section>
</div>
