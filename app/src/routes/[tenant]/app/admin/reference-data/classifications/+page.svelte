  ...
  <header class="hero section-card">
    <div>
      ...
      <p>
        ...
        release-scoped codes. Classification overlays canonical business identity; it never replaces
        it.
      </p>
    </div>
    <div class="principle">
      ...
      <small
        >Historical business assignments remain interpretable against the exact release used.</small
      >
    </div>
  </header>
    ...
    <aside class="systems section-card">
      <div class="panel-heading">
        <div>
          ...
          <h2>{data.systems.length} governed taxonomies</h2>
        </div>
      </div>
      ...
      {#if data.capabilities.canManage}
        <details class="command-panel">
          ...
          <form method="POST" action="?/createSystem">
            ...
            <label
              >External identifier<input
                name="systemIdentifier"
                placeholder="Optional source identifier"
              /></label
            >
            <label
              >Purpose<textarea name="purpose" rows="3" placeholder="Classification purpose"
              ></textarea></label
            >
    ...
    <main class="main-column">
        ...
        <section class="section-card releases">
          <div class="panel-heading">
            <div>
              ...
              <h2>{data.releases.length} releases</h2>
            </div>
            {#if data.capabilities.canManage}
              <details class="command-panel compact">
                ...
                <form method="POST" action="?/createRelease">
                  ...
                  <label
                    >Release key<input name="releaseKey" required placeholder="2026-09" /></label
                  >
                  ...
                  <label
                    >Source SHA-256<input
                      ...
                      minlength="64"
                      maxlength="64"
                      placeholder="64-character source digest"
                    /></label
                  >
          ...
          <div class="release-list">
            {#each data.releases as release}
              <a
                class:active={selectedRelease?.id === release.id}
                href={href(selectedSystem.id, release.id)}
              >
                <strong>{release.releaseKey}</strong>
                <span class={'release-status ' + release.status.toLowerCase()}
                  >{release.status}</span
                >
                <small
                  >{release.publicationDate
                    ? formatDate(release.publicationDate)
                    : 'Publication date not recorded'}</small
                >
              </a>
            {:else}
        ...
        {#if selectedRelease}
          <section class="section-card code-register">
            ...
            <div class="release-meta">
              ...
              <span
                ><small>Published</small><strong>{formatDate(selectedRelease.publishedAt)}</strong
                ></span
              >
              <span
                ><small>Digest</small><strong
                  >{selectedRelease.sourceDigestAlgorithm} · {selectedRelease.sourceDigest.slice(
                    0,
                    12
                  )}…</strong
                ></span
            ...
            {#if selectedRelease.status === 'DRAFT' && data.capabilities.canManage}
              <details class="command-panel import-panel">
                ...
                <form method="POST" action="?/importCodes">
                  ...
                  <p>
                    Paste up to 5,000 tab- or pipe-delimited rows. Parent order does not matter.
                ...
                <tbody>
                  {#each data.codes as code}
                    <tr>
                      ...
                      <td
                        >{code.title}{#if code.description}<small>{code.description}</small
                          >{/if}</td
                      >
                  ...
                  {:else}
                    <tr
                      ><td colspan="4" class="empty-cell">No codes match this release/search.</td
                      ></tr
                    >
                  {/each}
                </tbody>
      ...
      {:else}
        <section class="section-card empty-state">
          ...
          <p>
            Use the governed classification runtime for Uniclass and every other external or
            internal taxonomy.
          </p>
        </section>
      {/if}
...
<style>
  ...
  .system-list a.active,
  .release-list a.active {
    border-color: #79bde2;
    background: #edf8fe;
