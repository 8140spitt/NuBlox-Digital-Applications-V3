<script lang="ts">
  import {
    ...
    knowledgeRecordsCommunicationsRelationships,
    knowledgeRecordsCommunicationsRules
  } from '$lib/data/knowledge-records-communications-model';

  let { data } = $props();
  const names = new Map(knowledgeRecordsCommunicationsModel.map((x) => [x.modelId, x.canonicalName]));
  const external = new Map([
    ['REF-RECORD-TYPE','Record Type'],['REF-DATA-RETENTION-CLASS','Data Retention Class'],
    ['EVID-RETENTION-DISPOSITION-DECISION','Retention Disposition Decision'],
    ['EVID-LEGAL-HOLD-LINK','Legal Hold Link'],['CBO-INFORMATION-CONTAINER','Information Container'],
    ['CBO-PARTY','Party']
  ]);
  const nameFor=(id:string)=>names.get(id)??external.get(id)??id;
</script>

<svelte:head><title>Knowledge, Records & Communications Semantics · NuBlox</title></svelte:head>

<div class="page">
  <header class="hero card">
    <div>
      <span class="eyebrow">Knowledge and records architecture</span>
      <h1>Knowledge, Records, Communications & Stakeholder Engagement</h1>
      <p>Reusable knowledge, record declaration/retention, publication, communications and stakeholder engagement around the same canonical information and Party identities.</p>
    </div>
    <div class="actions">
      ...
      <a href={'/'+data.tenantSlug+'/app/admin/business-objects/legal-privacy'}>Legal & privacy</a>
      <strong>{knowledgeRecordsCommunicationsModel.length} governed constructs</strong>
      <span>{knowledgeRecordsCommunicationsRelationships.length} relationships</span>
    </div>
  </header>

  <section class="card chain">
    <div><span>Knowledge</span><strong>Article → Collection → Reuse</strong></div>
    <div><span>Records</span><strong>Declaration → Series/File → Retention</strong></div>
    <div><span>Disposition</span><strong>Eligibility → Hold → Request → Decision</strong></div>
    <div><span>Communications</span><strong>Plan → Item → Controlled Content</strong></div>
    <div><span>Stakeholders</span><strong>Plan → Engagement → Evidence</strong></div>
  </section>

  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Semantic contracts</span><h2>Governed knowledge, records and communications constructs</h2></div><p>Records and publications remain source-linked rather than copied into parallel repositories.</p></div>
    <div class="grid">
      {#each knowledgeRecordsCommunicationsModel as item,i}
        <details open={i<4}>
          <summary><span><code>{item.modelId}</code><strong>{item.canonicalName}</strong></span><small>{item.kind}</small></summary>
          <div class="body">
            <p>{item.definition}</p>
            <dl>
              <div><dt>Candidate</dt><dd>{item.candidateKeys.join(', ')}</dd></div>
  ...
  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Relationship graph</span><h2>Source-to-record-to-engagement provenance</h2></div></div>
    <div class="table"><table><thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Governance</th></tr></thead><tbody>
      {#each knowledgeRecordsCommunicationsRelationships as r}<tr><td><code>{r.id}</code></td><td>{nameFor(r.from)}</td><td>{r.predicate}</td><td>{nameFor(r.to)}</td><td>{r.governance}</td></tr>{/each}
    </tbody></table></div>
  </section>

  <section class="card content rules">
    <span class="eyebrow">Non-negotiable rules</span><h2>What NuBlox must preserve</h2>
    <ol>{#each knowledgeRecordsCommunicationsRules as x}<li>{x}</li>{/each}</ol>
  </section>
</div>
