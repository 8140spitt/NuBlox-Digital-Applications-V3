<script lang="ts">
  import { qhseAssuranceModel, qhseAssuranceRelationships, qhseAssuranceRules } from '$lib/data/qhse-assurance-model';
  let { data } = $props();
...
<div class="page">
  <header class="hero card">
    <div><span class="eyebrow">Canonical assurance model</span><h1>Quality, Health, Safety, Environment & Assurance</h1><p>Shared plans, verification, nonconformance, safe-work controls, incidents, compliance and environmental evidence without duplicating project, worker, asset or information identities.</p></div>
    <div class="actions"><a href={`/${data.tenantSlug}/app/admin/business-objects`}>← Canonicalization workbench</a><a href={`/${data.tenantSlug}/app/admin/business-objects/coverage-audit`}>Coverage audit</a><strong>{qhseAssuranceModel.length} governed constructs</strong><span>{qhseAssuranceRelationships.length} relationships</span></div>
  </header>
  ...
  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Semantic contracts</span><h2>Governed QHSE constructs</h2></div><p>Definitions, execution, evidence, cases and projections have independent identities and lifecycles.</p></div>
    <div class="grid">
      {#each qhseAssuranceModel as item, i}
        <details open={i < 4}><summary><span><code>{item.modelId}</code><strong>{item.canonicalName}</strong></span><small>{item.kind}</small></summary>
          <div class="body"><p>{item.definition}</p><dl><div><dt>Candidate</dt><dd>{item.candidateKeys.join(', ')}</dd></div><div><dt>Identity</dt><dd>{item.identityRule}</dd></div><div><dt>Lifecycle</dt><dd>{item.lifecycle.join(' · ')}</dd></div><div><dt>Governance</dt><dd><ul>{#each item.governance as x}<li>{x}</li>{/each}</ul></dd></div></dl></div>
        </details>
      {/each}
    </div>
  </section>
  ...
  <section class="card content">
    <div class="heading"><div><span class="eyebrow">Relationship graph</span><h2>Assurance flow</h2></div></div>
    <div class="table"><table><thead><tr><th>ID</th><th>From</th><th>Relationship</th><th>To</th><th>Governance</th></tr></thead><tbody>
      {#each qhseAssuranceRelationships as r}<tr><td><code>{r.id}</code></td><td>{names.get(r.from)}</td><td>{r.predicate}</td><td>{names.get(r.to)}</td><td>{r.governance}</td></tr>{/each}
    </tbody></table></div>
  </section>

  <section class="card content rules"><span class="eyebrow">Non-negotiable rules</span><h2>What NuBlox must preserve</h2><ol>{#each qhseAssuranceRules as x}<li>{x}</li>{/each}</ol></section>
</div>
<style>
