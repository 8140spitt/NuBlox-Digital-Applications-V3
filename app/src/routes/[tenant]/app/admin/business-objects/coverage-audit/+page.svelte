...
<div class="page">
  <header class="hero section-card">
    <div>
      ...
      <p>Machine-checkable proof that canonical discovery, convergence, benchmark challenge, aggregate ownership and all 1,510 enterprise activities are governed before controlled physical implementation.</p>
    </div>
  ...
  <section class="metrics">
    ...
    <article class="section-card"><span>Benchmark gaps</span><strong>{coverageAuditSummary.externalBenchmark.benchmarkGapResolvedCount}/{coverageAuditSummary.externalBenchmark.benchmarkGapCount}</strong><small>resolved · {coverageAuditSummary.externalBenchmark.benchmarkGapOpenCount} open</small></article>
    <article class="section-card"><span>Standards</span><strong>{coverageAuditSummary.externalBenchmark.standardsChallengedCount}/{coverageAuditSummary.externalBenchmark.standardsChallengeCount}</strong><small>challenged · {coverageAuditSummary.externalBenchmark.standardsOpenCount} open</small></article>
    <article class="section-card"><span>Rejected patterns</span><strong>{coverageAuditSummary.externalBenchmark.rejectedVendorPatternRecordedCount}/{coverageAuditSummary.externalBenchmark.rejectedVendorPatternCount}</strong><small>rationales recorded · {coverageAuditSummary.externalBenchmark.rejectedVendorPatternOpenCount} open</small></article>
    <article class="section-card"><span>Aggregate freeze</span><strong>{coverageAuditSummary.aggregateFreeze.aggregateBoundaryCount}</strong><small>boundaries · {coverageAuditSummary.aggregateFreeze.frozenFamilyCount}/29 families · {coverageAuditSummary.aggregateFreeze.state}</small></article>
    <article class="section-card"><span>Activity mapping</span><strong>{coverageAuditSummary.activityMapping.mappedActivityCount}/{coverageAuditSummary.activityMapping.sourceActivityCount}</strong><small>{coverageAuditSummary.activityMapping.mappedSubfunctionCount}/353 L2 · {coverageAuditSummary.activityMapping.unmappedSubfunctionCount} unmapped</small></article>
  </section>
  ...
  <section class="section-card lenses">
    ...
    <div class="lens-grid">
      ...
      <article><strong>2 · 29 workspaces</strong><p>Every function is represented and the final activity map covers 353/353 L2 sub-functions and 1,510/1,510 source activities against canonical objects/actions.</p></article>
      ...
      <article><strong>5 · External benchmark</strong><p>{coverageAuditSummary.externalBenchmark.name}: {coverageAuditSummary.externalBenchmark.challengedBenchmarks}/{coverageAuditSummary.externalBenchmark.registeredBenchmarks} market suites challenged across {coverageAuditSummary.externalBenchmark.requiredDomains} workspaces; {coverageAuditSummary.externalBenchmark.standardsChallengedCount}/{coverageAuditSummary.externalBenchmark.standardsChallengeCount} standards challenged; {coverageAuditSummary.externalBenchmark.benchmarkGapResolvedCount}/{coverageAuditSummary.externalBenchmark.benchmarkGapCount} findings resolved; {coverageAuditSummary.externalBenchmark.rejectedVendorPatternRecordedCount}/{coverageAuditSummary.externalBenchmark.rejectedVendorPatternCount} rejected patterns have recorded rationale. SAP: {coverageAuditSummary.externalBenchmark.sapV3MappedRows}/{coverageAuditSummary.externalBenchmark.legacySapCapabilityRows} remapped and {coverageAuditSummary.externalBenchmark.sapV3ChallengedRows}/{coverageAuditSummary.externalBenchmark.legacySapCapabilityRows} challenged. Gate 3: {coverageAuditSummary.externalBenchmark.state}.</p></article>
    </div>
  </section>
    ...
    {:else}
      <div class="gap-grid">
        <article>
          ...
          <small>Duplicate/alias convergence, Gate 3, the refined 174-boundary aggregate freeze and the 1,510-activity object/action mapping are closed.</small>
        </article>
      </div>
    {/if}
  </section>
