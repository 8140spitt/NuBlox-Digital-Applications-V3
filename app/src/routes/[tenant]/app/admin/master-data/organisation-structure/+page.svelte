...
<div class="structure-page">
    ...
    {#if data.canManage}
      <details class="create-panel">
        ...
        <form method="POST" action="?/create">
          ...
          <label
            >Unit type<input name="unitType" required placeholder="DIVISION, DEPARTMENT…" /></label
          >
          <label>
  ...
  <section class="metrics">
    <div class="metric section-card">
      <strong>{data.units.length}</strong><span>organisation units</span>
    </div>
    <div class="metric section-card"><strong>{activeCount}</strong><span>active units</span></div>
    <div class="metric section-card">
      <strong>{data.hierarchy.filter((edge) => edge.status === 'ACTIVE').length}</strong><span
        >active hierarchy links</span
      >
    </div>
    <div class="metric section-card">
      <strong>{data.legalEntities.length}</strong><span>legal entities</span>
    </div>
  </section>
  ...
  <div class="workspace-grid">
    <section class="unit-register section-card">
      <div class="panel-heading">
        <div>
          <span class="eyebrow">Structure register</span>
        ...
        {#each data.units as unit}
          ...
          >
            ...
            <span class="unit-copy"
              ><strong>{unit.name}</strong><small>{unit.unitType} · {unit.status}</small></span
            >
    ...
    <main class="inspector">
      {#if data.selected}
        <section class="section-card unit-card">
          ...
          <dl>
            <div>
              <dt>Accountable legal entity</dt>
              <dd>{data.selected.accountableLegalEntityName || 'Not assigned'}</dd>
            </div>
            <div>
              <dt>Valid from</dt>
              <dd>{data.selected.validFrom}</dd>
            </div>
            <div>
              <dt>Valid to</dt>
              <dd>{data.selected.validTo || 'Open-ended'}</dd>
            </div>
            <div>
              <dt>Current parent</dt>
              <dd>{data.activeParent?.parentUnitName || 'Root unit'}</dd>
            </div>
          </dl>
          ...
          {#if data.canManage && data.selected.status !== 'CLOSED'}
            <details class="edit-panel">
              ...
              <form method="POST" action="?/save">
                ...
                <label
                  >Unit code<input name="unitCode" value={data.selected.unitCode} required /></label
                >
                <label>Unit name<input name="name" value={data.selected.name} required /></label>
                <label
                  >Unit type<input name="unitType" value={data.selected.unitType} required /></label
                >
                <label>
                  Accountable legal entity
                  <select name="accountableLegalEntityPartyId">
                ...
                <label
                  >Valid from<input
                    type="date"
                    name="validFrom"
                    value={data.selected.validFrom.slice(0, 10)}
                  /></label
                >
                <label
                  >Valid to<input
                    type="date"
                    name="validTo"
                    value={data.selected.validTo?.slice(0, 10) ?? ''}
                  /></label
                >
        ...
        <section class="section-card hierarchy-card">
          <div class="panel-heading">
            <div>
              <span class="eyebrow">Effective hierarchy</span>
                ...
                <label>
                  ...
                  <select name="parentUnitId" required>
                    ...
                    {#each data.units.filter((unit) => unit.id !== data.selected.id && unit.status !== 'CLOSED') as unit}
                      <option
                        value={unit.id}
                        selected={data.activeParent?.parentUnitId === unit.id}
                      >
        ...
        <section class="section-card history-card">
          <div class="panel-heading">
            <div>
              <span class="eyebrow">Hierarchy evidence</span>
      ...
      {:else}
        <section class="section-card empty-state">
          ...
          <p>
            Organisation structure is kept separate from legal organisation identity and can evolve
...
<style>
  ...
  .hierarchy-actions form {
    display: grid;
    grid-template-columns: minmax(0, 1fr) 220px auto;
    gap: 8px;
    align-items: end;
  }
  .hierarchy-actions .remove-parent {
