...
describe('organisation change, transformation and continuous improvement semantic model', () => {
  it('is internally valid and covers all 23 BOF-26 candidates', () => {
    ...
    const decided = new Set(
      transformationProcessImprovementCanonicalization.map((x) => x.candidateKey)
    );
  ...
  it('reuses shared portfolio, action and communication semantics', () => {
    expect(
      transformationProcessImprovementCanonicalization.find((x) => x.candidateKey === 'BOF-26-001')
        ?.targetCandidateKey
    ).toBe('BOF-06-001');
    expect(
      transformationProcessImprovementCanonicalization.find((x) => x.candidateKey === 'BOF-26-005')
        ?.targetCandidateKey
    ).toBe('BOF-06-024');
    expect(
      transformationProcessImprovementCanonicalization.find((x) => x.candidateKey === 'BOF-26-006')
        ?.targetCandidateKey
    ).toBe('BOF-25-012');
  });

  it('keeps change learning planning separate from HCM learning truth', () => {
    const d = transformationProcessImprovementCanonicalization.find(
      (x) => x.candidateKey === 'BOF-26-008'
    );
  ...
  it('models process identity, model and version separately', () => {
    expect(
      transformationProcessImprovementModel.find((x) => x.modelId === 'PROC-ENTERPRISE-PROCESS')
        ?.kind
    ).toBe('process');
    expect(
      transformationProcessImprovementModel.find((x) => x.modelId === 'PROC-MODEL')?.kind
    ).toBe('controlled-definition');
    expect(
      transformationProcessImprovementCanonicalization.find((x) => x.candidateKey === 'BOF-26-015')
        ?.decision
    ).toBe('CHILD');
  });
  ...
  it('reuses responsibility, KPI, controlled information and compliance assessment', () => {
    expect(
      transformationProcessImprovementCanonicalization.find((x) => x.candidateKey === 'BOF-26-016')
        ?.targetCandidateKey
    ).toBe('BOF-06-020');
    expect(
      transformationProcessImprovementCanonicalization.find((x) => x.candidateKey === 'BOF-26-017')
        ?.targetCandidateKey
    ).toBe('BOF-02-008');
    expect(
      transformationProcessImprovementCanonicalization.find((x) => x.candidateKey === 'BOF-26-021')
        ?.targetCandidateKey
    ).toBe('BOF-07-007');
    expect(
      transformationProcessImprovementCanonicalization.find((x) => x.candidateKey === 'BOF-26-022')
        ?.targetCandidateKey
    ).toBe('BOF-21-007');
  });
